...
class AdvancedMetrics:
    ...
    def calculate_all_metrics(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in ['ret_30d', 'ret_3m']):
            with np.errstate(divide='ignore', invalid='ignore'):
                daily_ret_30d_comp = np.where(df['ret_30d'].fillna(0) != 0, df['ret_30d'].fillna(0) / 30, 0)
                daily_ret_3m_comp = np.where(df['ret_3m'].fillna(0) != 0, df['ret_3m'].fillna(0) / 90, 0)
...
class PatternDetector:
    ...
    def _get_all_pattern_definitions(df: pd.DataFrame) -> List[Tuple[str, Optional[pd.Series]]]:
        ...
        if all(col in df.columns for col in ['ret_7d', 'ret_30d', 'acceleration_score', 'rvol']):
            with np.errstate(divide='ignore', invalid='ignore'):
                daily_7d_pace = np.where(df['ret_7d'].fillna(0) != 0, df['ret_7d'].fillna(0) / 7, 0)
                daily_30d_pace = np.where(df['ret_30d'].fillna(0) != 0, df['ret_30d'].fillna(0) / 30, 0)
            mask = (
        ...
        if all(col in df.columns for col in ['vol_ratio_90d_180d', 'vol_ratio_30d_90d', 'from_low_pct', 'ret_7d', 'ret_30d']):
            with np.errstate(divide='ignore', invalid='ignore'):
                ret_ratio = np.where(df['ret_30d'].fillna(0) != 0, df['ret_7d'].fillna(0) / (df['ret_30d'].fillna(0) / 4), 0)
            mask = (
                (df['vol_ratio_90d_180d'] > 1.1) &
        ...
        if all(col in df.columns for col in ['ret_1d', 'ret_7d', 'rvol', 'from_high_pct', 'category']):
            with np.errstate(divide='ignore', invalid='ignore'):
                daily_pace_ratio = np.where(df['ret_7d'].fillna(0) != 0, df['ret_1d'].fillna(0) / (df['ret_7d'].fillna(0) / 7), 0)
            mask = (
                (daily_pace_ratio > 2) &
                (df['rvol'] > 3) &
                (df['from_high_pct'] > -15) &
...
class MarketIntelligence:
    ...
    @staticmethod
    def _dynamic_sample(group_df, group_size):
        if 1 <= group_size <= 5:
        ...
        if sample_count > 0:
            return group_df.nlargest(sample_count, 'master_score')
        return pd.DataFrame()
    ...
    def detect_sector_rotation(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for sector in df['sector'].unique():
            if sector != 'Unknown':
                ...
                sampled_df = MarketIntelligence._dynamic_sample(sector_df, sector_size)
                if not sampled_df.empty:
                    sector_dfs.append(sampled_df)
        if sector_dfs:
    ...
    def detect_industry_rotation(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for industry in df['industry'].unique():
            if industry != 'Unknown':
                ...
                sampled_df = MarketIntelligence._dynamic_sample(industry_df, industry_size)
                if not sampled_df.empty:
                    industry_dfs.append(sampled_df)
        if industry_dfs:
...
class FilterEngine:
    ...
    def apply_filters(df: pd.DataFrame, filters: Dict[str, Any]) -> pd.DataFrame:
        ...
        if patterns and 'patterns' in df.columns:
            pattern_regex = '|'.join([re.escape(p) for p in patterns]) # Escape patterns to avoid regex issues
    ...
    def create_excel_report(df: pd.DataFrame, template: str = 'full') -> BytesIO:
        ...
        try:
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                intel_data.append({
                    'Metric': 'Market Regime', 'Value': regime, 'Details': f"Breadth: {regime_metrics.get('breadth', 0):.1%}"
                })
                ad_metrics = MarketIntelligence.calculate_advance_decline_ratio(df)
                intel_data.append({
                    'Metric': 'Advance/Decline', 'Value': f"{ad_metrics.get('advancing', 0)}/{ad_metrics.get('declining', 0)}", 'Details': f"Ratio: {ad_metrics.get('ad_ratio', 1):.2f}"
                })
                ...
                wave_signals = df[
                    (df['momentum_score'] >= 60) & (df['acceleration_score'] >= 70) & (df['rvol'] >= 2)
                ].head(50)
...
class UIComponents:
    ...
    def render_summary_section(df: pd.DataFrame) -> None:
        ...
        with opp_col1:
            ready_to_run = df[
                (df['momentum_score'] >= 70) & (df['acceleration_score'] >= 70) & (df['rvol'] >= 2)
        ...
        with intel_col1:
            ...
            if not sector_rotation.empty:
                ...
                fig.add_trace(go.Bar(
                    ...
                    hovertemplate=(
                        'Sector: %{x}<br>' 'Flow Score: %{y:.1f}<br>'
        ...
        with intel_col2:
            ...
            strength_score = (
                (breadth * 50) + (min(avg_rvol, 2) * 25) + ((pattern_count / len(df)) * 25)
            )
...
class SessionStateManager:
    ...
    def initialize():
        defaults = {
            'search_query': "", 'last_refresh': datetime.now(timezone.utc), 'data_source': "sheet",
            'user_preferences': {
                'default_top_n': CONFIG.DEFAULT_TOP_N, 'display_mode': 'Technical', 'last_filters': {}
            },
            'filters': {}, 'active_filter_count': 0, 'quick_filter': None, 'quick_filter_applied': False,
            'show_debug': False, 'performance_metrics': {}, 'data_quality': {}, 'trigger_clear': False,
            'spreadsheet_id': "", 'category_filter': [], 'sector_filter': [], 'industry_filter': [],
            'min_score': 0, 'patterns': [], 'trend_filter': "All Trends", 'eps_tier_filter': [],
            'pe_tier_filter': [], 'price_tier_filter': [], 'min_eps_change': "", 'min_pe': "", 'max_pe': "",
            'require_fundamental_data': False, 'wave_states_filter': [], 'wave_strength_range_slider': (0, 100),
            'show_sensitivity_details': False, 'show_market_regime': True, 'wave_timeframe_select': "All Waves",
    ...
    def clear_filters():
        filter_keys = [
            'category_filter', 'sector_filter', 'industry_filter', 'eps_tier_filter', 'pe_tier_filter',
            'price_tier_filter', 'patterns', 'min_score', 'trend_filter', 'min_eps_change',
            'min_pe', 'max_pe', 'require_fundamental_data', 'quick_filter', 'quick_filter_applied',
            'wave_states_filter', 'wave_strength_range_slider', 'show_sensitivity_details',
            'show_market_regime', 'wave_timeframe_select', 'wave_sensitivity',
        ]
        for key in filter_keys:
            if key in st.session_state:
                if isinstance(st.session_state[key], list):
                    ...
                    if key in ['trend_filter', 'wave_timeframe_select', 'wave_sensitivity']:
                        st.session_state[key] = {'trend_filter': 'All Trends', 'wave_timeframe_select': 'All Waves', 'wave_sensitivity': 'Balanced'}.get(key)
                    else:
                        st.session_state[key] = ""
...
def main():
    ...
    st.markdown("""
    <style>
    /* ... (CSS styles are kept as they are) ... */
    ...
    with st.sidebar:
        ...
        if st.session_state.data_source == "upload":
            ...
            st.session_state.spreadsheet_id = st.text_input(
                "Enter Google Spreadsheet ID", value=st.session_state.get('spreadsheet_id', ""),
                placeholder="e.g. 1OEQ_qxL4lXbO9LlKWDGlD...",
                help="Enter the unique part of your Google Sheets URL.", key="spreadsheet_id_input"
            )
    ...
    try:
        if st.session_state.data_source == "upload" and uploaded_file is None:
            ...
            st.stop()
        if st.session_state.data_source == "sheet" and not st.session_state.get('spreadsheet_id'):
        ...
        with st.spinner("📥 Loading and processing data..."):
            try:
                if st.session_state.data_source == "upload" and uploaded_file is not None:
                    ...
                    ranked_df, data_timestamp, metadata = load_and_process_data("sheet", spreadsheet_id=st.session_state.get('spreadsheet_id'))
    ...
    with st.sidebar:
        ...
        filters['min_score'] = st.slider(
            "Minimum Master Score", min_value=0, max_value=100,
        ...
        filters['trend_filter'] = st.selectbox(
            "Trend Quality", options=list(trend_options.keys()), index=current_trend_index, key="trend_filter",
        ...
        filters['wave_states'] = st.multiselect(
            ...
            placeholder="Select wave states (empty = All)", help="Filter by the detected 'Wave State'",
        ...
        if 'overall_wave_strength' in ranked_df_display.columns:
            ...
            default_range_value = (int(min_strength) if pd.notna(min_strength) else 0,
                                   int(max_strength) if pd.notna(max_strength) else 100)
        ...
        with st.expander("🔧 Advanced Filters"):
            ...
            if 'eps_change_pct' in ranked_df_display.columns:
                eps_change_input = st.text_input("Min EPS Change %", value=st.session_state.get('min_eps_change', ""),
            ...
            if show_fundamentals and 'pe' in ranked_df_display.columns:
                ...
                col1, col2 = st.columns(2)
                with col1:
                    min_pe_input = st.text_input("Min PE Ratio", value=st.session_state.get('min_pe', ""),
                        placeholder="e.g. 10", key="min_pe"
                    )
                ...
                with col2:
                    max_pe_input = st.text_input("Max PE Ratio", value=st.session_state.get('max_pe', ""),
                        placeholder="e.g. 30", key="max_pe"
                    )
                ...
                filters['require_fundamental_data'] = st.checkbox(
                    "Only show stocks with PE and EPS data", value=st.session_state.get('require_fundamental_data', False),
    ...
    with col4:
        if show_fundamentals and 'eps_change_pct' in filtered_df.columns:
            ...
            growth_count, strong_count = positive_eps_growth.sum(), strong_growth.sum()
    ...
    with col6:
        if 'trend_quality' in filtered_df.columns:
            ...
            UIComponents.render_metric_card("Strong Trends", f"{strong_trends}",
    ...
    with tabs[0]:
        ...
        if not filtered_df.empty:
            ...
            with download_cols[0]:
                ...
                st.download_button(
                    label="📥 Download Filtered Data (CSV)", data=csv_filtered,
                    file_name=f"wave_detection_filtered_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv",
                    mime="text/csv", help="Download currently filtered stocks with all scores and indicators"
                )
            with download_cols[1]:
                ...
                st.download_button(
                    label="📥 Download Top 100 (CSV)", data=csv_top100,
                    file_name=f"wave_detection_top100_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv",
                    mime="text/csv", help="Download top 100 stocks by Master Score"
                )
            with download_cols[2]:
                ...
                if len(pattern_stocks) > 0:
                    ...
                    st.download_button(
                        label="📥 Download Pattern Stocks (CSV)", data=csv_patterns,
                        file_name=f"wave_detection_patterns_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.csv",
                        mime="text/csv", help="Download only stocks showing patterns"
                    )
    ...
    with tabs[2]:
        ...
        with radar_col2:
            sensitivity = st.select_slider(
                ...
                key="wave_sensitivity", help="Conservative = Stronger signals, Aggressive = More signals"
            )
        ...
        with radar_col4:
            if not wave_filtered_df.empty and 'overall_wave_strength' in wave_filtered_df.columns:
                try:
                    ...
                    UIComponents.render_metric_card(
                        "Wave Strength", f"{wave_emoji} {wave_strength_score:.0f}%", f"{wave_color} Market"
                    )
                except Exception as e:
        ...
        if show_sensitivity_details:
            with st.expander("📊 Current Sensitivity Thresholds", expanded=True):
                if sensitivity == "Conservative":
                    st.markdown("""
                    **Conservative Settings** 🛡️ - **Momentum Shifts:** Score ≥ 60, Acceleration ≥ 70
                    - **Emerging Patterns:** Within 5% of qualifying threshold - **Volume Surges:** RVOL ≥ 3.0x
                    - **Acceleration Alerts:** Score ≥ 85 - **Pattern Distance:** 5% from qualification
                    """)
                elif sensitivity == "Balanced":
                    st.markdown("""
                    **Balanced Settings** ⚖️ - **Momentum Shifts:** Score ≥ 50, Acceleration ≥ 60
                    - **Emerging Patterns:** Within 10% of qualifying threshold - **Volume Surges:** RVOL ≥ 2.0x
                    - **Acceleration Alerts:** Score ≥ 70 - **Pattern Distance:** 10% from qualification
                    """)
                else:
                    st.markdown("""
                    **Aggressive Settings** 🚀 - **Momentum Shifts:** Score ≥ 40, Acceleration ≥ 50
                    - **Emerging Patterns:** Within 15% of qualifying threshold - **Volume Surges:** RVOL ≥ 1.5x
                    - **Acceleration Alerts:** Score ≥ 60 - **Pattern Distance:** 15% from qualification
                    """)
        ...
        if wave_timeframe != "All Waves":
            try:
                if wave_timeframe == "Intraday Surge":
                    ...
                    if all(col in wave_filtered_df.columns for col in required_cols):
                        wave_filtered_df = wave_filtered_df[
                            (wave_filtered_df['ret_7d'] > 8) & (wave_filtered_df['vol_ratio_7d_90d'] > 2.0) &
                    ...
                    if all(col in wave_filtered_df.columns for col in required_cols):
                        wave_filtered_df = wave_filtered_df[
                            (wave_filtered_df['ret_30d'] > 15) & (wave_filtered_df['price'] > wave_filtered_df['sma_20d']) &
                            (wave_filtered_df['sma_20d'] > wave_filtered_df['sma_50d']) &
                            (wave_filtered_df['vol_ratio_30d_180d'] > 1.2) & (wave_filtered_df['from_low_pct'] > 30)
                        ]
            except Exception as e:
        ...
        if not wave_filtered_df.empty:
            ...
            if len(momentum_shifts) > 0:
                ...
                momentum_shifts['shift_strength'] = (
                    momentum_shifts['momentum_score'] * 0.4 + momentum_shifts['acceleration_score'] * 0.4 +
                        ...
                        if 'category' in wave_filtered_df.columns:
                            ...
                            if not normalized_cat_df.empty:
                                category_flow = normalized_cat_df.groupby('category').agg({
                                    'master_score': ['mean', 'count'], 'momentum_score': 'mean',
                                    'volume_score': 'mean', 'rvol': 'mean'
                                }).round(2)
                                if not category_flow.empty:
                                    category_flow.columns = ['Avg Score', 'Count', 'Avg Momentum', 'Avg Volume', 'Avg RVOL']
                                    category_flow['Flow Score'] = (
                                        category_flow['Avg Score'] * 0.4 + category_flow['Avg Momentum'] * 0.3 +
                                    ...
                                    fig_flow.add_trace(go.Bar(
                                        x=category_flow.index, y=category_flow['Flow Score'],
                                        text=[f"{val:.1f}" for val in category_flow['Flow Score']], textposition='outside',
                                        marker_color=['#2ecc71' if score > 60 else '#e74c3c' if score < 40 else '#f39c12'
                                    ...
                                    fig_flow.update_layout(
                                        ...
                                        xaxis_title="Market Cap Category", yaxis_title="Flow Score",
            ...
            if 'category_percentile' in wave_filtered_df.columns:
                ...
                for _, stock in close_to_leader.iterrows():
                    emergence_data.append({
                        'Ticker': stock['ticker'], 'Company': stock['company_name'], 'Pattern': '🔥 CAT LEADER',
            ...
            if 'breakout_score' in wave_filtered_df.columns:
                ...
                for _, stock in close_to_breakout.iterrows():
                    emergence_data.append({
                        'Ticker': stock['ticker'], 'Company': stock['company_name'], 'Pattern': '🎯 BREAKOUT',
            ...
            if len(volume_surges) > 0:
                volume_surges['surge_score'] = (
                    volume_surges['rvol_score'] * 0.5 + volume_surges['volume_score'] * 0.3 +
        ...
        if not filtered_df.empty:
            ...
            with col2:
                ...
                if pattern_counts:
                    ...
                    fig_patterns = go.Figure([
                        go.Bar(
                            x=pattern_df['Count'], y=pattern_df['Pattern'], orientation='h',
                            marker_color='#3498db', text=pattern_df['Count'], textposition='outside'
                        )
                    ])
                    fig_patterns.update_layout(
                        title="Pattern Frequency Analysis", xaxis_title="Number of Stocks",
                        yaxis_title="Pattern", template='plotly_white', height=400, margin=dict(l=150)
                    )
            ...
            if not sector_overview_df_local.empty:
                ...
                sector_overview_display['Coverage %'] = (
                    ...
                    .replace([np.inf, -np.inf], np.nan).fillna(0).round(1).apply(lambda x: f"{x}%")
                )
            ...
            if not industry_overview_df_local.empty:
                ...
                st.info("📊 **Normalized Analysis**: Shows metrics for dynamically sampled stocks per industry to ensure fair comparison across industries of different sizes.")
    ...
    with tabs[4]:
        ...
        with col1:
            search_query = st.text_input(
                "Search stocks", placeholder="Enter ticker or company name...",
                help="Search by ticker symbol or company name", key="search_input"
            )
        with col2:
            ...
            if not search_results.empty:
                ...
                for _, stock in search_results.iterrows():
                    with st.expander(f"📊 {stock['ticker']} - {stock['company_name']} (Rank #{int(stock['rank'])})", expanded=True):
                        ...
                        with metric_cols[0]:
                            UIComponents.render_metric_card("Master Score", f"{stock['master_score']:.1f}", f"Rank #{int(stock['rank'])}")
                        with metric_cols[1]:
                        ...
                        with metric_cols[2]:
                            UIComponents.render_metric_card("From Low", f"{stock['from_low_pct']:.0f}%", "52-week range position")
                        with metric_cols[3]:
                        ...
                        with metric_cols[5]:
                            UIComponents.render_metric_card("Wave State", stock.get('wave_state', 'N/A'), stock['category'])
    ...
    with tabs[5]:
        ...
        template_map = {
            "Full Analysis (All Data)": "full", "Day Trader Focus": "day_trader",
            "Swing Trader Focus": "swing_trader", "Investor Focus": "investor"
        }
        ...
        with col1:
            ...
            st.markdown(
                ...
                "- Sector and Industry rotation analysis\n"
                ...
                if len(filtered_df) == 0: st.error("No data to export. Please adjust your filters.")
                    ...
                    with st.spinner("Creating Excel report..."):
                        try:
                            ...
                            st.download_button(
                                label="📥 Download Excel Report", data=excel_file,
            ...
            if st.button("Generate CSV Export", use_container_width=True):
                if len(filtered_df) == 0: st.error("No data to export. Please adjust your filters.")
                    ...
                    try:
                        ...
                        st.download_button(
                            label="📥 Download CSV File", data=csv_data,
    ...
    with tabs[6]:
        ...
        with col2:
            ...
            #### 📈 Pattern Groups
            **Technical Patterns**
            - 🔥 CAT LEADER - 💎 HIDDEN GEM - 🚀 ACCELERATING
            - 🏦 INSTITUTIONAL - ⚡ VOL EXPLOSION - 🎯 BREAKOUT
            - 👑 MARKET LEADER - 🌊 MOMENTUM WAVE - 💰 LIQUID LEADER
            - 💪 LONG STRENGTH - 📈 QUALITY TREND
            **Range Patterns**
            - 🎯 52W HIGH APPROACH - 🔄 52W LOW BOUNCE - 👑 GOLDEN ZONE
            - 📊 VOL ACCUMULATION - 🔀 MOMENTUM DIVERGE - 🎯 RANGE COMPRESS
            **NEW Intelligence**
            - 🤫 STEALTH - 🧛 VAMPIRE - ⛈️ PERFECT STORM
            **Fundamental** (Hybrid)
            - 💎 VALUE MOMENTUM - 📊 EARNINGS ROCKET - 🏆 QUALITY LEADER
            - ⚡ TURNAROUND - ⚠️ HIGH PE
            #### ⚡ Performance
            - Initial load: <2 seconds - Filtering: <200ms
            - Pattern detection: <500ms - Search: <50ms
            - Export: <1 second
            #### 🔒 Production Status
            ...
            **Indian Market Optimized**
            - ₹ Currency formatting - IST timezone aware
            - NSE/BSE categories - Local number formats
            """)
        st.markdown("---")
        st.markdown("#### 📊 Current Session Statistics")
        stats_cols = st.columns(4)
        with stats_cols[0]:
            UIComponents.render_metric_card("Total Stocks Loaded", f"{len(ranked_df):,}" if 'ranked_df' in locals() else "0")
        with stats_cols[1]:
            UIComponents.render_metric_card("Currently Filtered", f"{len(filtered_df):,}" if 'filtered_df' in locals() else "0")
        with stats_cols[2]:
        ...
        with stats_cols[3]:
            ...
            cache_status, cache_emoji = ("Fresh", "🟢") if minutes < 60 else ("Stale", "🔴")
