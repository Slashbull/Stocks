...
Enhanced with a perfect filtering system and robust error handling
"""
...
class SessionStateManager:
    ...
    def initialize():
        ...
        defaults = {
            ...
            'data_source': "sheet",
            'sheet_url': "https://docs.google.com/spreadsheets/d/1OEQ_qxL4lXbO9LlKWDGlDju2yQC1iYvOYeXF3mTQuJM/edit?usp=sharing",
            'gid': "1823439984",
            'user_preferences': {
                'default_top_n': 50,
...
# ============================================
# SMART CACHING WITH VERSIONING - FIXED
# ============================================
...
# ============================================
# PATTERN DETECTION ENGINE - CORRECTED
# ============================================
...
class UIComponents:
    ...
    def render_summary_section(df: pd.DataFrame) -> None:
        ...
        with col1:
            ...
            if ad_ratio > 2: ad_emoji = "🔥"; ad_color = "inverse"
            elif ad_ratio > 1: ad_emoji = "📈"; ad_color = "normal"
            else: ad_emoji = "📉"; ad_color = "off"
            
            UIComponents.render_metric_card(
                "A/D Ratio", f"{ad_emoji} {ad_ratio:.2f}",
        ...
        with col2:
            ...
            UIComponents.render_metric_card(
                "Momentum Health", f"{momentum_pct:.0f}%", f"{high_momentum} strong stocks"
            )
        ...
        with col3:
            ...
            UIComponents.render_metric_card(
                "Volume State", f"{vol_emoji} {avg_rvol:.1f}x", f"{high_vol_count} surges"
            )
        ...
        with intel_col1:
            ...
            if not sector_rotation.empty:
                ...
                fig.add_trace(go.Bar(
                    ...
                    hovertemplate=('Sector: %{x}<br>Flow Score: %{y:.1f}<br>Analyzed: %{customdata[0]} of %{customdata[1]} stocks<br>'
                                   'Avg Score: %{customdata[2]:.1f}<br>Median Score: %{customdata[3]:.1f}<extra></extra>'),
                    customdata=np.column_stack((
                        sector_rotation['analyzed_stocks'][:10], sector_rotation['total_stocks'][:10],
                        sector_rotation['avg_score'][:10], sector_rotation['median_score'][:10]
                    ))
                ))
                fig.update_layout(
                    title="Sector Rotation Map - Smart Money Flow (Dynamically Sampled)",
                    xaxis_title="Sector", yaxis_title="Flow Score", height=400,
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.data_source == "upload":
            uploaded_file = st.file_uploader(
                "Choose CSV file", type="csv", help="Upload a CSV file with stock data. Must contain 'ticker' and 'price' columns."
            )
            ...
            sheet_url_input = st.text_input(
                ...
                help="Enter the full Google Sheets URL. Must be publicly accessible.",
                key="sheet_url"
            )
    ...
    try:
        ...
        if st.session_state.data_source == "sheet" and not sheet_url: st.warning("Please enter a Google Sheets URL to continue"); st.stop()
        ...
        with st.spinner("📥 Loading and processing data..."):
            try:
                if st.session_state.data_source == "upload" and uploaded_file is not None:
                    ...
                    ranked_df, data_timestamp, metadata = load_and_process_data("sheet", sheet_url=sheet_url, gid=gid)
    ...
    with st.sidebar:
        ...
        display_mode_index = 0 if st.session_state.user_preferences.get('display_mode') == 'Technical' else 1
        ...
        selected_categories = st.multiselect(
            "Market Cap Category", options=categories, default=st.session_state.get('category_filter', []),
        ...
        selected_sectors = st.multiselect(
            "Sector", options=sectors, default=st.session_state.get('sector_filter', []),
        ...
        if 'industry' in ranked_df_display.columns:
            ...
            selected_industries = st.multiselect(
                "Industry", options=industries, default=st.session_state.get('industry_filter', []),
                placeholder="Select industries (empty = All)", key="industry_filter"
            )
            filters['industries'] = selected_industries
        
        filters['min_score'] = st.slider(
            "Minimum Master Score", min_value=0, max_value=100, value=st.session_state.get('min_score', 0),
            step=5, help="Filter stocks by minimum score", key="min_score"
        )
        ...
        if all_patterns:
            filters['patterns'] = st.multiselect(
                "Patterns", options=sorted(all_patterns), default=st.session_state.get('patterns', []),
        ...
        filters['wave_states'] = st.multiselect(
            "Wave State", options=wave_states_options, default=st.session_state.get('wave_states_filter', []),
        ...
        if 'overall_wave_strength' in ranked_df_display.columns:
            ...
            filters['wave_strength_range'] = st.slider(
                ...
                value=current_slider_value, step=1,
                help="Filter by the calculated 'Overall Wave Strength' score", key="wave_strength_range_slider"
            )
        ...
        with st.expander("🔧 Advanced Filters"):
            for tier_type, col_name in [('eps_tiers', 'eps_tier'), ('pe_tiers', 'pe_tier'), ('price_tiers', 'price_tier')]:
                if col_name in ranked_df_display.columns:
                    ...
                    selected_tiers = st.multiselect(
                        ...
                        default=st.session_state.get(f'{col_name}_filter', []), placeholder=f"Select {col_name.replace('_', ' ')}s (empty = All)",
            ...
            if 'eps_change_pct' in ranked_df_display.columns:
                eps_change_input = st.text_input(
                    "Min EPS Change %", value=st.session_state.get('min_eps_change', ""),
                    placeholder="e.g. -50 or leave empty", help="Enter minimum EPS growth percentage", key="min_eps_change"
                )
            ...
            if show_fundamentals and 'pe' in ranked_df_display.columns:
                ...
                filters['require_fundamental_data'] = st.checkbox(
                    "Only show stocks with PE and EPS data", value=st.session_state.get('require_fundamental_data', False), key="require_fundamental_data"
                )
    ...
    active_filter_count = st.session_state.active_filter_count
    if active_filter_count > 0 or quick_filter_applied:
        filter_status_col1, filter_status_col2 = st.columns([5, 1])
        with filter_status_col1:
            if quick_filter:
                quick_filter_names = {'top_gainers': '📈 Top Gainers', 'volume_surges': '🔥 Volume Surges',
    ...
    with tabs[1]:
        ...
        with col1:
            ...
            display_count = st.selectbox(
                "Show top", options=CONFIG.AVAILABLE_TOP_N,
        ...
        if not display_df.empty:
            ...
            display_cols.update({
                'from_low_pct': 'From Low', 'ret_30d': '30D Ret', 'rvol': 'RVOL', 'vmi': 'VMI',
                'patterns': 'Patterns', 'category': 'Category'
            })
            ...
            with st.expander("📊 Quick Statistics"):
                ...
                with stat_cols[0]:
                    ...
                    if 'master_score' in filtered_df.columns:
                        st.text(f"Max: {filtered_df['master_score'].max():.1f}"); st.text(f"Min: {filtered_df['master_score'].min():.1f}")
                        st.text(f"Mean: {filtered_df['master_score'].mean():.1f}"); st.text(f"Median: {filtered_df['master_score'].median():.1f}")
                        st.text(f"Q1: {filtered_df['master_score'].quantile(0.25):.1f}"); st.text(f"Q3: {filtered_df['master_score'].quantile(0.75):.1f}")
                        st.text(f"Std: {filtered_df['master_score'].std():.1f}")
                with stat_cols[1]:
                    st.markdown("**Returns (30D)**")
                    if 'ret_30d' in filtered_df.columns:
                        st.text(f"Max: {filtered_df['ret_30d'].max():.1f}%"); st.text(f"Min: {filtered_df['ret_30d'].min():.1f}%")
                        st.text(f"Avg: {filtered_df['ret_30d'].mean():.1f}%"); st.text(f"Positive: {(filtered_df['ret_30d'] > 0).sum()}")
                ...
                with stat_cols[2]:
                    if show_fundamentals:
                        ...
                        if 'rvol' in filtered_df.columns:
                            st.text(f"Max: {filtered_df['rvol'].max():.1f}x"); st.text(f"Avg: {filtered_df['rvol'].mean():.1f}x")
                ...
                with stat_cols[3]:
                    ...
                    if 'trend_quality' in filtered_df.columns:
                        ...
                        st.text(f"Avg Trend Score: {avg_trend_score:.1f}"); st.text(f"Above All SMAs: {stocks_above_all_smas}")
                        st.text(f"In Uptrend (60+): {stocks_in_uptrend}"); st.text(f"In Downtrend (<40): {stocks_in_downtrend}")
    ...
    with tabs[2]:
        ...
        with radar_col1:
            wave_timeframe = st.selectbox(
                "Wave Detection Timeframe", options=["All Waves", "Intraday Surge", "3-Day Buildup", "Weekly Breakout", "Monthly Trend"],
        ...
        with radar_col2:
            sensitivity = st.select_slider(
                "Detection Sensitivity", options=["Conservative", "Balanced", "Aggressive"],
                value=st.session_state.get('wave_sensitivity', "Balanced"), key="wave_sensitivity",
                help="Conservative = Stronger signals, Aggressive = More signals"
            )
            show_sensitivity_details = st.checkbox(
                "Show thresholds", value=st.session_state.get('show_sensitivity_details', False),
                key="show_sensitivity_details", help="Display exact threshold values for current sensitivity"
            )
        with radar_col3:
            show_market_regime = st.checkbox(
                "📊 Market Regime Analysis", value=st.session_state.get('show_market_regime', True),
                key="show_market_regime", help="Show category rotation flow and market regime detection"
            )
        ...
        with radar_col4:
            if not wave_filtered_df.empty and 'overall_wave_strength' in wave_filtered_df.columns:
                ...
                except Exception as e: logger.error(f"Error calculating wave strength: {str(e)}"); UIComponents.render_metric_card("Wave Strength", "N/A", "Error")
            ...
            try:
                if wave_timeframe == "Intraday Surge":
                    ...
                    if all(col in wave_filtered_df.columns for col in ['ret_7d', 'vol_ratio_7d_90d', 'from_high_pct']):
                        wave_filtered_df = wave_filtered_df[(wave_filtered_df['ret_7d'] > 8) & (wave_filtered_df['vol_ratio_7d_90d'] > 2.0) & (wave_filtered_df['from_high_pct'] > -10)]
                elif wave_timeframe == "Monthly Trend":
                    if all(col in wave_filtered_df.columns for col in ['ret_30d', 'price', 'sma_20d', 'sma_50d', 'vol_ratio_30d_180d', 'from_low_pct']):
                        wave_filtered_df = wave_filtered_df[(wave_filtered_df['ret_30d'] > 15) & (wave_filtered_df['price'] > wave_filtered_df['sma_20d']) & (wave_filtered_df['sma_20d'] > wave_filtered_df['sma_50d']) & (wave_filtered_df['vol_ratio_30d_180d'] > 1.2) & (wave_filtered_df['from_low_pct'] > 30)]
            except Exception as e: logger.warning(f"Error applying {wave_timeframe} filter: {str(e)}"); st.warning(f"Some data not available for {wave_timeframe} filter")
        ...
        if not wave_filtered_df.empty:
            ...
            if len(accelerating_stocks) > 0:
                ...
                with col1: perfect_accel = len(accelerating_stocks[accelerating_stocks['acceleration_score'] >= 90]); st.metric("Perfect Acceleration (90+)", perfect_accel)
                with col2: strong_accel = len(accelerating_stocks[accelerating_stocks['acceleration_score'] >= 80]); st.metric("Strong Acceleration (80+)", strong_accel)
                with col3: avg_accel = accelerating_stocks['acceleration_score'].mean(); st.metric("Avg Acceleration Score", f"{avg_accel:.1f}")
                        ...
                        if 'category' in wave_filtered_df.columns:
                            ...
                            if not normalized_cat_df.empty:
                                category_flow = normalized_cat_df.groupby('category').agg({
                                    'master_score': ['mean', 'count'], 'momentum_score': 'mean', 'volume_score': 'mean', 'rvol': 'mean'
                                }).round(2)
                                if not category_flow.empty:
                                    category_flow.columns = ['Avg Score', 'Count', 'Avg Momentum', 'Avg Volume', 'Avg RVOL']
                                    category_flow['Flow Score'] = (category_flow['Avg Score'] * 0.4 + category_flow['Avg Momentum'] * 0.3 + category_flow['Avg Volume'] * 0.3)
                                    ...
                                    if 'Small' in top_category or 'Micro' in top_category: flow_direction = "🔥 RISK-ON"
                                    elif 'Large' in top_category or 'Mega' in top_category: flow_direction = "❄️ RISK-OFF"
                                    else: flow_direction = "➡️ Neutral"
                                    fig_flow = go.Figure()
                                    fig_flow.add_trace(go.Bar(
                                        x=category_flow.index, y=category_flow['Flow Score'], text=[f"{val:.1f}" for val in category_flow['Flow Score']],
                                        textposition='outside', marker_color=['#2ecc71' if score > 60 else '#e74c3c' if score < 40 else '#f39c12' for score in category_flow['Flow Score']],
                                        hovertemplate='Category: %{x}<br>Flow Score: %{y:.1f}<br>Stocks: %{customdata}<extra></extra>', customdata=category_flow['Count']
                                    ))
                                    fig_flow.update_layout(
                                        title=f"Smart Money Flow Direction: {flow_direction} (Dynamically Sampled)",
                                        xaxis_title="Market Cap Category", yaxis_title="Flow Score", height=300,
                                        template='plotly_white', showlegend=False
                                    )
            ...
            if 'category_percentile' in wave_filtered_df.columns:
                ...
                for _, stock in close_to_leader.iterrows():
                    emergence_data.append({'Ticker': stock['ticker'], 'Company': stock['company_name'], 'Pattern': '🔥 CAT LEADER',
                                           'Distance': f"{90 - stock['category_percentile']:.1f}% away",
                                           'Current': f"{stock['category_percentile']:.1f}%ile", 'Score': stock['master_score']})
            if 'breakout_score' in wave_filtered_df.columns:
                ...
                for _, stock in close_to_breakout.iterrows():
                    emergence_data.append({'Ticker': stock['ticker'], 'Company': stock['company_name'], 'Pattern': '🎯 BREAKOUT',
                                           'Distance': f"{80 - stock['breakout_score']:.1f} pts away",
                                           'Current': f"{stock['breakout_score']:.1f} score", 'Score': stock['master_score']})
            if emergence_data:
    ...
    with tabs[3]:
        ...
        if not filtered_df.empty:
            col1, col2 = st.columns(2)
            with col1:
                fig_dist = Visualizer.create_score_distribution(filtered_df); st.plotly_chart(fig_dist, use_container_width=True)
            with col2:
                pattern_counts = {}
                ...
                if not industry_overview_df.empty:
                    ...
                    with ind_tab1:
                        ...
                        fig_industry.add_trace(go.Bar(
                            x=top_industries.index[:15], y=top_industries['flow_score'][:15],
                            text=[f"{val:.1f}" for val in top_industries['flow_score'][:15]], textposition='outside',
                            marker_color=['#2ecc71' if score > 60 else '#e74c3c' if score < 40 else '#f39c12' for score in top_industries['flow_score'][:15]],
                            hovertemplate=('Industry: %{x}<br>Flow Score: %{y:.1f}<br>Analyzed: %{customdata[0]} of %{customdata[1]} stocks<br>'
                                           'Sampling: %{customdata[2]:.1f}%<br>Avg Score: %{customdata[3]:.1f}<br>Median Score: %{customdata[4]:.1f}<extra></extra>'),
                            customdata=np.column_stack((top_industries['analyzed_stocks'][:15], top_industries['total_stocks'][:15],
                                                        top_industries['sampling_pct'][:15], top_industries['avg_score'][:15],
                                                        top_industries['median_score'][:15]))
                        ))
                        fig_industry.update_layout(
                            title="Top 15 Industries by Smart Money Flow", xaxis_title="Industry",
                            yaxis_title="Flow Score", height=500, template='plotly_white', showlegend=False,
                            xaxis_tickangle=-45
                        )
                        st.plotly_chart(fig_industry, use_container_width=True)
                        col1, col2, col3, col4 = st.columns(4)
                        with col1: UIComponents.render_metric_card("Total Industries", f"{len(industry_overview_df):,}")
                    ...
                    with ind_tab2:
                        ...
                        display_names = {
                            'flow_score': 'Flow Score', 'avg_score': 'Avg Score', 'median_score': 'Median Score',
                            'avg_momentum': 'Avg Momentum', 'avg_volume': 'Avg Volume', 'avg_rvol': 'Avg RVOL',
                            'avg_ret_30d': 'Avg 30D Ret', 'analyzed_stocks': 'Analyzed', 'total_stocks': 'Total',
                            'sampling_pct': 'Sample %'
                        }
                        ...
                        st.info("""... Smart Dynamic Sampling details ...""")
            ...
            if not category_overview_df.empty:
                ...
                with cat_tab1:
                    ...
                    colors = {'Mega Cap': '#1f77b4', 'Large Cap': '#2ca02c', 'Mid Cap': '#ff7f0e', 'Small Cap': '#d62728', 'Micro Cap': '#9467bd'}
                    bar_colors = [colors.get(cat, '#7f7f7f') for cat in category_overview_df.index]
                    fig_category.add_trace(go.Bar(
                        x=category_overview_df.index, y=category_overview_df['flow_score'],
                        text=[f"{val:.1f}" for val in category_overview_df['flow_score']], textposition='outside',
                        marker_color=bar_colors,
                        hovertemplate=('Category: %{x}<br>Flow Score: %{y:.1f}<br>Analyzed: %{customdata[0]} of %{customdata[1]} stocks<br>'
                                       'Avg Score: %{customdata[2]:.1f}<br>Avg Momentum: %{customdata[3]:.1f}<br>'
                                       'Avg Acceleration: %{customdata[4]:.1f}<extra></extra>'),
                        customdata=np.column_stack((category_overview_df['analyzed_stocks'], category_overview_df['total_stocks'],
                                                    category_overview_df['avg_score'], category_overview_df['avg_momentum'],
                                                    category_overview_df['avg_acceleration']))
                    ))
                    if len(category_overview_df) >= 3:
                        small_micro_avg = category_overview_df.loc[category_overview_df.index.isin(['Small Cap', 'Micro Cap']), 'flow_score'].mean()
                        large_mega_avg = category_overview_df.loc[category_overview_df.index.isin(['Large Cap', 'Mega Cap']), 'flow_score'].mean()
                    ...
                    else: market_state = "📊 ANALYZING..."
                    fig_category.update_layout(
                        title=f"Category Performance - {market_state}", xaxis_title="Market Cap Category",
                        yaxis_title="Flow Score", height=400, template='plotly_white', showlegend=False
                    )
                    ...
                    with col1:
                        ...
                        UIComponents.render_metric_card("Leading Category", f"{best_category}", f"Score: {best_score:.1f}")
                    with col2:
                        if 'avg_momentum' in category_overview_df.columns:
                            highest_momentum = category_overview_df.nlargest(1, 'avg_momentum')
                            if not highest_momentum.empty: UIComponents.render_metric_card("Highest Momentum", f"{highest_momentum.index[0]}", f"{highest_momentum['avg_momentum'].iloc[0]:.1f}")
                    with col3:
                        if 'avg_acceleration' in category_overview_df.columns:
                            highest_accel = category_overview_df.nlargest(1, 'avg_acceleration')
                            if not highest_accel.empty: UIComponents.render_metric_card("Best Acceleration", f"{highest_accel.index[0]}", f"{highest_accel['avg_acceleration'].iloc[0]:.1f}")
                with cat_tab2:
            ...
            if not search_results.empty:
                ...
                for idx, stock in search_results.iterrows():
                    with st.expander(f"📊 {stock['ticker']} - {stock['company_name']} (Rank #{int(stock['rank'])})", expanded=True):
                        ...
                        with metric_cols[3]:
                            ret_30d = stock.get('ret_30d', 0); UIComponents.render_metric_card("30D Return", f"{ret_30d:+.1f}%", "↑" if ret_30d > 0 else "↓")
                        with metric_cols[4]:
                            rvol = stock.get('rvol', 1); UIComponents.render_metric_card("RVOL", f"{rvol:.1f}x", "High" if rvol > 2 else "Normal")
                        ...
                        for i, (name, score, weight) in enumerate(components):
                            with score_cols[i]:
                                color, display_score = ("⚪", "N/A") if pd.isna(score) else ("🟢", f"{score:.0f}") if score >= 80 else ("🟡", f"{score:.0f}") if score >= 60 else ("🔴", f"{score:.0f}")
                        ...
                        with detail_cols_top[0]:
                            st.markdown("**📊 Classification**"); st.text(f"Sector: {stock.get('sector', 'Unknown')}")
                            if 'industry' in stock: st.text(f"Industry: {stock.get('industry', 'Unknown')}")
                            st.text(f"Category: {stock.get('category', 'Unknown')}")
                            if show_fundamentals:
                                st.markdown("**💰 Fundamentals**")
                                if 'pe' in stock and pd.notna(stock['pe']):
                                    pe_val = stock['pe']; st.text(f"PE Ratio: {'🔴 Loss' if pe_val <= 0 else f'🟢 {pe_val:.1f}x' if pe_val < 15 else f'🟡 {pe_val:.1f}x' if pe_val < 25 else f'🔴 {pe_val:.1f}x'}")
                                ...
                                if 'eps_change_pct' in stock and pd.notna(stock['eps_change_pct']):
                                    eps_chg = stock['eps_change_pct']; st.text(f"EPS Growth: {'🚀' if eps_chg >= 100 else '🔥' if eps_chg >= 50 else '📈' if eps_chg >= 0 else '📉'} {eps_chg:+.1f}%")
    ...
    with tabs[5]:
        ...
        export_template = st.radio(
            "Choose export template:", options=["Full Analysis (All Data)", "Day Trader Focus", "Swing Trader Focus", "Investor Focus"],
            key="export_template_radio", help="Select a template based on your trading style"
        )
    ...
    with tabs[6]:
        ...
        with col2:
            st.markdown("... [Pattern Groups and Stats] ...")
        st.markdown("---")
        ...
        with stats_cols[3]:
            ...
            cache_status = "Fresh" if minutes < 60 else "Stale"; cache_emoji = "🟢" if minutes < 60 else "🔴"
