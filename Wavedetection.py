...
class PatternDetector:
    ...
    def detect_all_patterns(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for item in patterns_with_masks_raw:
            # Ensure the tuple format is (name, mask)
            if isinstance(item[0], str): # Correct format is (name, mask)
                ...
                # This logic applies if _get_all_pattern_definitions
                # sometimes returns (mask, name) instead of (name, mask)
                # based on the original V2 code where this order was sometimes mixed.
...
class SessionStateManager:
    ...
    def clear_filters():
        ...
        filter_keys = [
            ...
            'wave_states_filter', # NEW: Added for V4 bug fix
            'wave_strength_range_slider' # NEW: Added for V4 bug fix
        ]
...
def main():
    ...
    with st.sidebar:
        ...
        if 'overall_wave_strength' in ranked_df_display.columns:
            ...
            if min_strength == max_strength:
                ...
                # Ensure default value for slider also respects adjusted min/max
                default_range_value = (int(min_strength), int(max_strength)) if int(min_strength) <= int(max_strength) else (slider_min_val, slider_max_val)
            else:
                slider_min_val = 0
                slider_max_val = 100
    ...
    with tabs[3]:
        ...
        if not filtered_df.empty:
            ...
            sector_rotation_old_tab = MarketIntelligence.detect_sector_rotation(filtered_df)
