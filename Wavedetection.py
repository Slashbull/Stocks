...
class DataValidator:
    ...
    @staticmethod
    def clean_numeric_value(value: Any, is_percentage: bool = False, bounds: Optional[Tuple[float, float]] = None) -> Tuple[Optional[float], bool]:
        """Clean and convert numeric values with bounds checking, returning a flag for modification"""
    ...
    try:
        if source_type == "upload" and file_data is not None:
            ...
            if not spreadsheet_id:
                raise ValueError("A Google Spreadsheet ID is required.")
            ...
            try:
                df = pd.read_csv(csv_url, low_memory=False, timeout=10)
                ...
                if 'last_good_data' in st.session_state:
                    logger.info("Using cached data from session state as fallback")
        ...
        df, modifications = DataProcessor.process_dataframe(df, metadata)
        metadata['data_modifications'] = modifications
...
class DataProcessor:
    ...
    def process_dataframe(df: pd.DataFrame, metadata: Dict[str, Any]) -> Tuple[pd.DataFrame, int]:
        ...
        for col in numeric_cols:
            if col in df.columns:
                ...
                cleaned_series, modified_flags = zip(*df[col].apply(lambda x: DataValidator.clean_numeric_value(x, is_pct, bounds)))
                df[col] = pd.Series(cleaned_series, index=df.index, dtype=float)
                data_modifications += sum(modified_flags)
    ...
    def calculate_all_metrics(df: pd.DataFrame) -> pd.DataFrame:
        ...
        if all(col in df.columns for col in ['ret_7d', 'ret_30d']):
            with np.errstate(divide='ignore', invalid='ignore'):
                daily_ret_7d = df['ret_7d'].div(7).fillna(0)
                daily_ret_30d = df['ret_30d'].div(30).fillna(0)
            df['momentum_harmony'] += (daily_ret_7d > daily_ret_30d).astype(int)
        
        if all(col in df.columns for col in ['ret_30d', 'ret_3m']):
            with np.errstate(divide='ignore', invalid='ignore'):
                daily_ret_30d_comp = df['ret_30d'].div(30).fillna(0)
                daily_ret_3m_comp = df['ret_3m'].div(90).fillna(0)
...
class RankingEngine:
    ...
    def _calculate_acceleration_score(df: pd.DataFrame) -> pd.Series:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            avg_daily_1d = ret_1d
            avg_daily_7d = ret_7d.div(7).fillna(0)
            avg_daily_30d = ret_30d.div(30).fillna(0)
    ...
    def _calculate_breakout_score(df: pd.DataFrame) -> pd.Series:
        ...
        has_from_high = 'from_high_pct' in df.columns
        has_vol_ratio = 'vol_ratio_7d_90d' in df.columns
        has_smas = any(col in df.columns for col in ['sma_20d', 'sma_50d', 'sma_200d'])
        
        if not (has_from_high and has_vol_ratio and has_smas):
            logger.warning("Insufficient data for breakout score, using neutral scores")
        ...
        if has_smas and 'price' in df.columns:
    ...
    def _calculate_trend_quality(df: pd.DataFrame) -> pd.Series:
        ...
        if not all(col in df.columns for col in sma_cols):
            logger.warning("Insufficient SMA data for trend calculation")
            return trend_score
        ...
        above_count = (
            (current_price > df['sma_20d']).astype(int) +
            (current_price > df['sma_50d']).astype(int) +
            (current_price > df['sma_200d']).astype(int)
        )
...
class PatternDetector:
    ...
    def detect_all_patterns(df: pd.DataFrame) -> pd.DataFrame:
        ...
        pattern_names_ordered = [name for name, _ in patterns_with_masks]
        
        pattern_matrix = np.zeros((len(df), num_patterns), dtype=bool)
        
        for i, (_, mask) in enumerate(patterns_with_masks):
    ...
    def _get_all_pattern_definitions(df: pd.DataFrame) -> List[Tuple[str, Optional[pd.Series]]]:
        ...
        if 'category_percentile' in df.columns:
            mask = safe_mask(df['category_percentile'] >= CONFIG.PATTERN_THRESHOLDS['category_leader'])
        ...
        if 'acceleration_score' in df.columns:
            mask = safe_mask(df['acceleration_score'] >= CONFIG.PATTERN_THRESHOLDS['acceleration'])
        ...
        if 'rvol' in df.columns:
            mask = safe_mask(df['rvol'] > 3)
            patterns.append(('⚡ VOL EXPLOSION', mask))
        
        # 6. Breakout Ready
        if 'breakout_score' in df.columns:
            mask = safe_mask(df['breakout_score'] >= CONFIG.PATTERN_THRESHOLDS['breakout_ready'])
            patterns.append(('🎯 BREAKOUT', mask))
        
        # 7. Market Leader
        if 'percentile' in df.columns:
            mask = safe_mask(df['percentile'] >= CONFIG.PATTERN_THRESHOLDS['market_leader'])
        ...
        if 'long_term_strength' in df.columns:
            mask = safe_mask(df['long_term_strength'] >= CONFIG.PATTERN_THRESHOLDS['long_strength'])
            patterns.append(('💪 LONG STRENGTH', mask))
        
        # 11. Quality Trend
        if 'trend_quality' in df.columns:
            mask = safe_mask(df['trend_quality'] >= 80)
            patterns.append(('📈 QUALITY TREND', mask))
        
        # 12. Value Momentum (Fundamental)
        if 'pe' in df.columns and 'percentile' in df.columns and 'master_score' in df.columns:
            has_valid_pe = safe_mask(df['pe'].notna() & (df['pe'] > 0) & (df['pe'] < 10000))
            mask = has_valid_pe & (df['pe'] < 15) & (df['master_score'] >= 70)
            patterns.append(('💎 VALUE MOMENTUM', mask))
        
        # 13. Earnings Rocket
        if 'eps_change_pct' in df.columns and 'acceleration_score' in df.columns:
            has_eps_growth = safe_mask(df['eps_change_pct'].notna())
            extreme_growth = safe_mask(df['eps_change_pct'] > 1000)
            normal_growth = safe_mask(df['eps_change_pct'] > 50) & safe_mask(df['eps_change_pct'] <= 1000)
            
            mask = (
                (extreme_growth & safe_mask(df['acceleration_score'] >= 80)) |
                (normal_growth & safe_mask(df['acceleration_score'] >= 70))
            )
            patterns.append(('📊 EARNINGS ROCKET', mask))
        
        # 14. Quality Leader
        if all(col in df.columns for col in ['pe', 'eps_change_pct', 'percentile']):
            has_complete_data = safe_mask(
                df['pe'].notna() & 
        ...
        if 'eps_change_pct' in df.columns and 'volume_score' in df.columns:
            has_eps = safe_mask(df['eps_change_pct'].notna())
            mega_turnaround = has_eps & safe_mask(df['eps_change_pct'] > 500) & safe_mask(df['volume_score'] >= 60)
            strong_turnaround = has_eps & safe_mask(df['eps_change_pct'] > 100) & safe_mask(df['eps_change_pct'] <= 500) & safe_mask(df['volume_score'] >= 70)
        ...
        # 16. High PE Warning
        if 'pe' in df.columns:
            has_valid_pe = safe_mask(df['pe'].notna() & (df['pe'] > 0))
            mask = has_valid_pe & safe_mask(df['pe'] > 100)
        ...
        if all(col in df.columns for col in ['ret_7d', 'ret_30d', 'acceleration_score', 'rvol']):
            with np.errstate(divide='ignore', invalid='ignore'):
                daily_7d_pace = df['ret_7d'].div(7).fillna(0)
                daily_30d_pace = df['ret_30d'].div(30).fillna(0)
        ...
        if all(col in df.columns for col in ['high_52w', 'low_52w', 'from_low_pct']):
            with np.errstate(divide='ignore', invalid='ignore'):
                range_pct = df['high_52w'].sub(df['low_52w']).div(df['low_52w']).mul(100).fillna(100)
            
            mask = safe_mask(range_pct < 50) & safe_mask(df['from_low_pct'] > 30)
            patterns.append(('🎯 RANGE COMPRESS', mask))
        
        # 23. Stealth Accumulator
        if all(col in df.columns for col in ['vol_ratio_90d_180d', 'vol_ratio_30d_90d', 'from_low_pct', 'ret_7d', 'ret_30d']):
            with np.errstate(divide='ignore', invalid='ignore'):
                ret_ratio = df['ret_7d'].div(df['ret_30d'].div(4)).fillna(0)
        ...
        if all(col in df.columns for col in ['ret_1d', 'ret_7d', 'rvol', 'from_high_pct', 'category']):
            with np.errstate(divide='ignore', invalid='ignore'):
                daily_pace_ratio = df['ret_1d'].div(df['ret_7d'].div(7)).fillna(0)
    ...
    def detect_sector_rotation(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for sector in df['sector'].unique():
            if sector != 'Unknown':
                ...
                if sample_count > 0 and 'master_score' in sector_df.columns:
    ...
    def detect_industry_rotation(df: pd.DataFrame) -> pd.DataFrame:
        ...
        for industry in df['industry'].unique():
            if industry != 'Unknown':
                ...
                if sample_count > 0 and 'master_score' in industry_df.columns:
    ...
    def render_summary_section(df: pd.DataFrame) -> None:
        ...
        with intel_col1:
            ...
            if not sector_rotation.empty:
                ...
                fig.update_layout(
                    title=f"Sector Rotation Map: {flow_direction} (Dynamically Sampled)",
                    xaxis_title="Sector",
...
class SessionStateManager:
    ...
    def initialize():
        ...
        defaults = {
            ...
            'data_quality': {},
            'trigger_clear': False,
            'user_spreadsheet_id': st.secrets.get('google_sheet_id', ''),
        ...
        # Ensure filters dictionary is always present
    ...
    def clear_filters():
        ...
        # Define the default state for filters in a single dictionary
        filter_defaults = {
            'category_filter': [],
            'sector_filter': [],
            'industry_filter': [],
        ...
        # Update the session state with these defaults
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.data_source == "upload":
            ...
            # Use st.secrets as the initial value for better security and persistence
            initial_sheet_id = st.secrets.get('google_sheet_id', '')
            if 'user_spreadsheet_id' not in st.session_state:
                st.session_state.user_spreadsheet_id = initial_sheet_id
            ...
            # Check if the input has changed from the last valid state
            if spreadsheet_id_input_value != st.session_state.user_spreadsheet_id:
    ...
    with st.sidebar:
        ...
        sectors = FilterEngine.get_filter_options(ranked_df_display, 'sector', st.session_state.filters)
        ...
        if 'industry' in ranked_df_display.columns:
            industries = FilterEngine.get_filter_options(ranked_df_display, 'industry', st.session_state.filters)
            ...
            st.session_state.filters['industries'] = selected_industries
        else:
        ...
        if all_patterns:
            st.session_state.filters['patterns'] = st.multiselect(
                "Patterns",
            ...
            st.session_state.filters['patterns'] = []
        ...
        st.session_state.filters['trend_filter'] = st.selectbox(
            "Trend Quality",
        ...
        st.session_state.filters['trend_range'] = trend_options[st.session_state.filters['trend_filter']]

        st.markdown("#### 🌊 Wave Filters")
        wave_states_options = FilterEngine.get_filter_options(ranked_df_display, 'wave_state', st.session_state.filters)
        st.session_state.filters['wave_states'] = st.multiselect(
            "Wave State",
        ...
        if 'overall_wave_strength' in ranked_df_display.columns:
            min_strength = ranked_df_display['overall_wave_strength'].min() if not ranked_df_display.empty else 0
            max_strength = ranked_df_display['overall_wave_strength'].max() if not ranked_df_display.empty else 100
            ...
            st.session_state.filters['wave_strength_range'] = st.slider(
            ...
            st.session_state.filters['wave_strength_range'] = (0, 100)
        ...
        with st.expander("🔧 Advanced Filters"):
            for tier_type, col_name in [
                ...
                if col_name in ranked_df_display.columns:
                    tier_options = FilterEngine.get_filter_options(ranked_df_display, col_name, st.session_state.filters)
                    ...
                    st.session_state.filters[tier_type] = selected_tiers
            ...
            if 'eps_change_pct' in ranked_df_display.columns:
                ...
                if eps_change_input.strip():
                    try:
                        st.session_state.filters['min_eps_change'] = float(eps_change_input)
                    except ValueError:
                        st.error("Please enter a valid number for EPS change")
                        st.session_state.filters['min_eps_change'] = None
                else:
                    st.session_state.filters['min_eps_change'] = None
            ...
            if show_fundamentals and 'pe' in ranked_df_display.columns:
                ...
                with col1:
                    ...
                    if min_pe_input.strip():
                        try:
                            st.session_state.filters['min_pe'] = float(min_pe_input)
                        except ValueError:
                            st.error("Invalid Min PE")
                            st.session_state.filters['min_pe'] = None
                    else:
                        st.session_state.filters['min_pe'] = None
                with col2:
                    ...
                    if max_pe_input.strip():
                        try:
                            st.session_state.filters['max_pe'] = float(max_pe_input)
                        except ValueError:
                            st.error("Invalid Max PE")
                            st.session_state.filters['max_pe'] = None
                    else:
                        st.session_state.filters['max_pe'] = None
                
                st.session_state.filters['require_fundamental_data'] = st.checkbox(
                    "Only show stocks with PE and EPS data",
                    value=st.session_state.get('require_fundamental_data', False),
                    key="require_fundamental_data"
                )
    
    filtered_df = FilterEngine.apply_filters(ranked_df, st.session_state.filters)
    filtered_df = filtered_df.sort_values('rank')
    
    st.session_state.user_preferences['last_filters'] = st.session_state.filters
    ...
    with tabs[2]:
        ...
        with radar_col4:
            if not wave_filtered_df.empty and 'overall_wave_strength' in wave_filtered_df.columns:
                ...
                if wave_strength_score > 70:
                    ...
                    wave_emoji = "💤"
                UIComponents.render_metric_card("Wave Strength", f"{wave_emoji} {wave_strength_score:.0f}%", "Market")
            else:
                ...
                if wave_timeframe == "Intraday Surge":
                    ...
                    if all(col in wave_filtered_df.columns for col in ['ret_30d', 'price', 'sma_20d', 'sma_50d', 'vol_ratio_30d_180d', 'from_low_pct']):
                        wave_filtered_df = wave_filtered_df[
                            ...
                            (wave_filtered_df['sma_20d'] > wave_filtered_df['sma_50d']) & (wave_filtered_df['vol_ratio_30d_180d'] > 1.2) &
        ...
        if not wave_filtered_df.empty:
            ...
            if len(momentum_shifts) > 0:
                ...
                display_columns = ['ticker', 'company_name', 'master_score', 'momentum_score', 'acceleration_score', 'rvol', 'signal_count', 'wave_state', 'category']
                ...
                shift_display = shift_display.rename(columns=rename_dict).drop('signal_count', axis=1)
            ...
            if len(accelerating_stocks) > 0:
                ...
                with col1:
                    st.metric("Perfect Acceleration (90+)", len(accelerating_stocks[accelerating_stocks['acceleration_score'] >= 90]))
                with col2:
                    st.metric("Strong Acceleration (80+)", len(accelerating_stocks[accelerating_stocks['acceleration_score'] >= 80]))
                with col3:
                    st.metric("Avg Acceleration Score", f"{accelerating_stocks['acceleration_score'].mean():.1f}")
            else:
                        ...
                        if 'category' in wave_filtered_df.columns:
                            ...
                            for cat in wave_filtered_df['category'].unique():
                                if cat != 'Unknown':
                                    ...
                                    if sample_count > 0 and not cat_df.empty:
                                ...
                                if not normalized_cat_df.empty:
                                    ...
                                    if not category_flow.empty:
                                        ...
                                        fig_flow.add_trace(go.Bar(
                                            ...
                                            marker_color=['#2ecc71' if score > 60 else '#e74c3c' if score < 40 else '#f39c12' for score in category_flow['Flow Score']],
            ...
            if len(volume_surges) > 0:
                ...
                with col1:
                    display_cols = ['ticker', 'company_name', 'rvol', 'price', 'money_flow_mm', 'wave_state', 'category', 'sector', 'industry']
                    ...
                    rename_dict = {'ticker': 'Ticker', 'company_name': 'Company', 'rvol': 'RVOL', 'price': 'Price', 'money_flow_mm': 'Money Flow', 'wave_state': 'Wave', 'category': 'Category', 'sector': 'Sector', 'industry': 'Industry', 'ret_1d': '1D Ret'}
                    ...
                    with st.expander(f"📊 {stock['ticker']} - {stock['company_name']} (Rank #{int(stock['rank'])})", expanded=True):
                        ...
                        with detail_cols_top[0]:
                            ...
                            if show_fundamentals:
                                ...
                                if 'pe' in stock and pd.notna(stock['pe']):
                                    pe_val = stock['pe']; st.text(f"PE Ratio: 🟢 {pe_val:.1f}x" if pe_val < 15 else f"PE Ratio: 🟡 {pe_val:.1f}x" if pe_val < 25 else f"PE Ratio: 🔴 {pe_val:.1f}x") if pe_val > 0 else st.text("PE Ratio: 🔴 Loss")
                            ...
                            for i, (sma_col, sma_label) in enumerate(sma_checks):
                                ...
                                with display_col:
                                    if sma_col in stock.index and pd.notna(stock[sma_col]) and stock[sma_col] > 0:
                                        sma_value = stock[sma_col]; pct_diff = ((current_price - sma_value) / sma_value) * 100
                        ...
                        with detail_cols_tech[1]:
                            ...
                            if 'trend_quality' in stock.index:
                                tq = stock['trend_quality']; st.markdown(f"🔥 Strong Uptrend ({tq:.0f})" if tq >= 80 else f"✅ Good Uptrend ({tq:.0f})" if tq >= 60 else f"➡️ Neutral Trend ({tq:.0f})" if tq >= 40 else f"⚠️ Weak/Downtrend ({tq:.0f})")
    ...
    with tabs[5]:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("#### 📊 Excel Report"); st.markdown("Comprehensive multi-sheet report including: - Top 100 stocks with all scores - Market intelligence dashboard - Sector rotation analysis - Pattern frequency analysis - Wave Radar signals - Summary statistics")
        ...
        export_stats = {"Total Stocks": len(filtered_df), "Average Score": f"{filtered_df['master_score'].mean():.1f}" if not filtered_df.empty else "N/A", "Stocks with Patterns": (filtered_df['patterns'] != '').sum() if 'patterns' in filtered_df.columns else 0, "High RVOL (>2x)": (filtered_df['rvol'] > 2).sum() if 'rvol' in filtered_df.columns else 0, "Positive 30D Returns": (filtered_df['ret_30d'] > 0).sum() if 'ret_30d' in filtered_df.columns else 0, "Data Quality": f"{st.session_state.data_quality.get('completeness', 0):.1f}%"}
    ...
    with tabs[6]:
        ...
        with stats_cols[2]: data_quality = st.session_state.data_quality.get('completeness', 0); quality_emoji = "🟢" if data_quality > 80 else "🟡" if data_quality > 60 else "🔴"; UIComponents.render_metric_card("Data Quality", f"{quality_emoji} {data_quality:.1f}%")
        with stats_cols[3]: cache_time = datetime.now(timezone.utc) - st.session_state.last_refresh; minutes = int(cache_time.total_seconds() / 60); cache_status = "Fresh" if minutes < 60 else "Stale"; cache_emoji = "🟢" if minutes < 60 else "🔴"; UIComponents.render_metric_card("Cache Age", f"{cache_emoji} {minutes} min", cache_status)
