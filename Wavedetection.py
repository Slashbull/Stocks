"""
Wave Detection Ultimate 3.1 - FINAL PERFECTED PRODUCTION VERSION
===============================================================
Professional Stock Ranking System with Advanced Analytics
All bugs fixed, optimized for Streamlit Community Cloud
Enhanced with perfect filtering system and robust error handling

Version: 3.1.0-FINAL-PERFECTED-REFACTORED
...
# ============================================
# ROBUST SESSION STATE MANAGER - FIXED & REFINED
# ============================================
...
class RobustSessionState:
    ...
    def clear_filters():
        ...
        filter_keys = [
            ...
            'min_score', 'trend_filter', 'min_eps_change_input',
        ...
        st.info("✅ All filters cleared! Displaying all data.")
...
class MarketIntelligence:
    ...
    def detect_market_regime(df: pd.DataFrame) -> Tuple[str, Dict[str, Any]]:
        ...
        if 'category' in df.columns and 'master_score' in df.columns:
            ...
            micro_small_avg = category_scores[category_scores.index.isin(['Micro Cap', 'Small Cap'])].mean() if not category_scores[category_scores.index.isin(['Micro Cap', 'Small Cap'])].empty else 50
            large_mega_avg = category_scores[category_scores.index.isin(['Large Cap', 'Mega Cap'])].mean() if not category_scores[category_scores.index.isin(['Large Cap', 'Mega Cap'])].empty else 50
    ...
    @staticmethod
    def _sample_for_rotation(df: pd.DataFrame, group_col: str, min_sample: int) -> pd.DataFrame:
        """Helper to sample top-performing stocks from each group for fair comparison"""
        ...
        for group in df[group_col].unique():
            if group != 'Unknown' and pd.notna(group):
                ...
                if group_size == 1:
                    ...
                    sample_count = max(min_sample, int(group_size * 0.80))
                elif 11 <= group_size <= 25:
                    sample_count = max(min_sample, int(group_size * 0.60))
                elif 26 <= group_size <= 50:
                    sample_count = max(min_sample, int(group_size * 0.40))
                elif 51 <= group_size <= 100:
                    sample_count = max(min_sample, int(group_size * 0.30))
                else: # > 100 stocks
                    sample_count = min(100, int(group_size * 0.20))
    ...
    def detect_sector_rotation(df: pd.DataFrame) -> pd.DataFrame:
        ...
        normalized_df = MarketIntelligence._sample_for_rotation(df, 'sector', min_sample=3)
        ...
        if isinstance(sector_metrics.columns, pd.MultiIndex):
    ...
    def detect_industry_rotation(df: pd.DataFrame) -> pd.DataFrame:
        ...
        normalized_df = MarketIntelligence._sample_for_rotation(df, 'industry', min_sample=2)
        if normalized_df.empty:
            return pd.DataFrame()
        ...
        if isinstance(industry_metrics.columns, pd.MultiIndex):
    ...
    def detect_category_performance(df: pd.DataFrame) -> pd.DataFrame:
        ...
        normalized_df = MarketIntelligence._sample_for_rotation(df, 'category', min_sample=2)
        if normalized_df.empty:
            return pd.DataFrame()
        ...
        if isinstance(category_metrics.columns, pd.MultiIndex):
...
class FilterEngine:
    ...
    def apply_filters(df: pd.DataFrame, filters: Dict[str, Any]) -> pd.DataFrame:
        ...
        if quick_filter == 'top_gainers':
            mask &= df['momentum_score'] >= 80
        elif quick_filter == 'volume_surges':
            mask &= df['rvol'] >= 3
        elif quick_filter == 'breakout_ready':
            mask &= df['breakout_score'] >= 80
        ...
        try:
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                if template in templates and templates[template]['columns']:
                    ...
                    export_cols = top_100.columns.tolist()
...
def main():
    ...
    st.set_page_config(
        page_title="Wave Detection Ultimate 3.1",
        page_icon="🌊",
        layout="wide",
    ...
    ">
        <h1 style="margin: 0; font-size: 2.5rem;">🌊 Wave Detection Ultimate 3.1</h1>
    ...
    with st.sidebar:
        ...
        filter_checks = [
            ...
            ('min_eps_change_input', lambda x: x is not None and str(x).strip() != ''),
            ('min_pe', lambda x: x is not None and x != 0.0),
            ('max_pe', lambda x: x is not None and x != float('inf')),
    ...
    with st.sidebar:
        ...
        filters['categories'] = st.multiselect(
            "Market Cap Category",
            options=FilterEngine.get_filter_options(ranked_df, 'category', {}),
            default=RobustSessionState.safe_get('category_filter', []),
            placeholder="Select categories (empty = All)",
            key="category_filter"
        )
        
        temp_filters_with_category = {'categories': filters['categories']}
        filters['sectors'] = st.multiselect(
            "Sector",
            options=FilterEngine.get_filter_options(ranked_df, 'sector', temp_filters_with_category),
            default=RobustSessionState.safe_get('sector_filter', []),
            placeholder="Select sectors (empty = All)",
            key="sector_filter"
        )
        
        temp_filters_with_sector = {'categories': filters['categories'], 'sectors': filters['sectors']}
        if 'industry' in ranked_df.columns:
            filters['industries'] = st.multiselect(
                "Industry",
                options=FilterEngine.get_filter_options(ranked_df, 'industry', temp_filters_with_sector),
        ...
        with st.expander("🔧 Advanced Filters"):
            if 'eps_tier' in ranked_df.columns:
                filters['eps_tiers'] = st.multiselect(
                    "EPS Tier",
                    options=FilterEngine.get_filter_options(ranked_df, 'eps_tier', filters),
            ...
            if 'pe_tier' in ranked_df.columns:
                filters['pe_tiers'] = st.multiselect(
                    "PE Tier",
                    options=FilterEngine.get_filter_options(ranked_df, 'pe_tier', filters),
            ...
            if 'price_tier' in ranked_df.columns:
                filters['price_tiers'] = st.multiselect(
                    "Price Tier",
                    options=FilterEngine.get_filter_options(ranked_df, 'price_tier', filters),
            ...
            if show_fundamentals and 'pe' in ranked_df.columns:
                ...
                min_pe_val = float(st.text_input(
                    "Min PE Ratio", value=str(RobustSessionState.safe_get('min_pe', "")).replace('inf', ''),
                    placeholder="e.g. 10", key="min_pe_input"
                ).strip() or 0)
                max_pe_val = float(st.text_input(
                    "Max PE Ratio", value=str(RobustSessionState.safe_get('max_pe', "")).replace('inf', ''),
                    placeholder="e.g. 30", key="max_pe_input"
                ).strip() or float('inf'))
                ...
                if 'eps_change_pct' in ranked_df.columns:
                    eps_change_input = st.text_input(
                        "Min EPS Change %",
                        value=RobustSessionState.safe_get('min_eps_change_input', ""),
            ...
            if 'wave_state' in ranked_df.columns:
                filters['wave_states'] = st.multiselect(
                    "Wave State",
                    options=FilterEngine.get_filter_options(ranked_df, 'wave_state', filters),
            ...
            if 'overall_wave_strength' in ranked_df.columns:
                min_strength = ranked_df['overall_wave_strength'].min()
                max_strength = ranked_df['overall_wave_strength'].max()
                default_range = (0, 100) if pd.isna(min_strength) or pd.isna(max_strength) else (int(min_strength), int(max_strength))
                
                filters['wave_strength_range'] = st.slider(
                    "Overall Wave Strength",
                    min_value=0, max_value=100,
                    value=RobustSessionState.safe_get('wave_strength_range_slider', default_range),
                    step=1,
    ...
    with tabs[1]:
        ...
        sort_by = st.selectbox("Sort by", options=sort_options, index=0, key='sort_by_select')
        ...
        if not display_df.empty:
            ...
            if not show_fundamentals:
                display_cols_map.pop('pe', None)
                display_cols_map.pop('eps_change_pct', None)

            available_display_cols = {c: n for c, n in display_cols_map.items() if c in display_df.columns}
            
            display_df = display_df[list(available_display_cols.keys())]
            display_df.columns = available_display_cols.values()
            ...
            with st.expander("📊 Quick Statistics"):
                ...
                with stat_cols[3]:
                    ...
                    if 'trend_quality' in filtered_df.columns:
                        ...
                        st.text(f"In Uptrend: {stocks_in_uptrend}")
                        st.text(f"In Downtrend: {stocks_in_downtrend}")
        else:
            ...
            try:
                if wave_timeframe == "Intraday Surge":
                    ...
                    wave_filtered_df = wave_filtered_df[
                        ...
                        (wave_filtered_df.get('trend_quality', 0) >= 60)
                    ]
            except Exception as e:
        ...
        if not wave_filtered_df.empty:
            ...
            if len(momentum_shifts) > 0:
                ...
                display_columns = ['ticker', 'company_name', 'master_score', 'momentum_score',
                                   'acceleration_score', 'rvol', 'signal_count', 'wave_state', 'category']
                
                if 'ret_7d' in top_shifts.columns:
                    display_columns.insert(5, 'ret_7d')
                ...
                shift_display = shift_display.rename(columns={
                    ...
                    'rvol': 'RVOL', 'ret_7d': '7D Ret', 'wave_state': 'Wave', 'category': 'Category'
                })
                
                shift_display = shift_display[['Ticker', 'Company', 'Score', 'Momentum', 'Acceleration', 'RVOL', '7D Ret', 'Signals', 'Wave', 'Category']]
        ...
        if not filtered_df.empty:
            ...
            if 'industry' in filtered_df.columns:
                ...
                if not industry_overview_df.empty:
                    ...
                    with ind_tab1:
                        ...
                        with col2:
                            top_3_avg = top_industries.head(3)['avg_score'].mean() if len(top_industries) >= 3 else 0
                            UIComponents.render_metric_card(
                                "Top 3 Avg Score",
                                f"{top_3_avg:.1f}"
                            )
                        ...
                        with col3:
                            ...
                            UIComponents.render_metric_card(
                                "Strong Industries",
                                f"{strong_industries}",
                                f"{strong_industries/len(industry_overview_df)*100:.0f}% of total" if len(industry_overview_df) > 0 else "0%"
                            )
                    ...
                    with ind_tab2:
                        ...
                        display_names = {
                            ...
                            'avg_ret_30d': 'Avg 30D Ret', 'analyzed_stocks': 'Analyzed',
            ...
            if not category_overview_df.empty:
                ...
                with cat_tab1:
                    ...
                    colors = {
                        'Mega Cap': '#1f77b4', 'Large Cap': '#2ca02c', 'Mid Cap': '#ff7f0e',
                        'Small Cap': '#d62728', 'Micro Cap': '#9467bd'
                    }
                    ...
                    if len(category_overview_df) >= 3:
                        ...
                        ].mean() if not category_overview_df.loc[category_overview_df.index.isin(['Small Cap', 'Micro Cap'])].empty else 0
                        large_mega_avg = category_overview_df.loc[
                            category_overview_df.index.isin(['Large Cap', 'Mega Cap']), 'flow_score'
                        ].mean() if not category_overview_df.loc[category_overview_df.index.isin(['Large Cap', 'Mega Cap'])].empty else 0
                ...
                with cat_tab2:
                    ...
                    display_names = {
                        'flow_score': 'Flow Score', 'avg_score': 'Avg Score', 'median_score': 'Median Score',
                        'avg_momentum': 'Avg Momentum', 'avg_acceleration': 'Avg Acceleration', 'avg_breakout': 'Avg Breakout',
                        'avg_volume': 'Avg Volume', 'avg_rvol': 'Avg RVOL', 'avg_ret_30d': 'Avg 30D Ret',
                        'analyzed_stocks': 'Analyzed', 'total_stocks': 'Total', 'sampling_pct': 'Sample %'
                    }
                    ...
                    with st.expander(
                        ...
                        with metric_cols[3]:
                            ...
                            UIComponents.render_metric_card(
                                "30D Return",
                                f"{ret_30d:+.1f}%",
                                "↑" if ret_30d > 0 else "↓" if ret_30d < 0 else "↔️"
                            )
                        ...
                        with detail_cols_top[1]:
                            ...
                            for period, col in [
                                ("1 Day", 'ret_1d'), ("7 Days", 'ret_7d'),
                                ("30 Days", 'ret_30d'), ("3 Months", 'ret_3m'),
                                ("6 Months", 'ret_6m'), ("1 Year", 'ret_1y')
                            ]:
    ...
    with tabs[6]:
        st.markdown("### ℹ️ About Wave Detection Ultimate 3.1 - Final Perfected Production Version")
        
        col1, col2 = st.columns([2, 1])
        
        with col1:
            st.markdown("""
            #### 🌊 Welcome to Wave Detection Ultimate 3.1
    ...
    st.markdown(
        ...
        <div style="text-align: center; color: #666; padding: 1rem;">
            🌊 Wave Detection Ultimate 3.1 - Final Perfected Production Version<br>
