...
def load_and_process_data(source_type: str, file_data=None, 
...
def main():
    ...
    with st.sidebar:
        ...
        if 'overall_wave_strength' in ranked_df_display.columns:
            min_strength = float(ranked_df_display['overall_wave_strength'].min()) if not ranked_df_display['overall_wave_strength'].empty else 0
            max_strength = float(ranked_df_display['overall_wave_strength'].max()) if not ranked_df_display['overall_wave_strength'].empty else 100
    ...
    with tabs[2]:
        ...
        if not wave_filtered_df.empty:
            ...
            if sensitivity == "Conservative":
                momentum_threshold = 60; acceleration_threshold = 70; min_rvol = 3.0
            elif sensitivity == "Balanced":
                momentum_threshold = 50; acceleration_threshold = 60; min_rvol = 2.0
            else:
                momentum_threshold = 40; acceleration_threshold = 50; min_rvol = 1.5
            ...
            if len(momentum_shifts) > 0:
                ...
                shift_display = shift_display.rename(columns={
                    'ticker': 'Ticker', 'company_name': 'Company', 'master_score': 'Score', 'momentum_score': 'Momentum',
                    'acceleration_score': 'Acceleration', 'wave_state': 'Wave', 'category': 'Category'})
                        ...
                        if 'category' in wave_filtered_df.columns:
                            ...
                            if not normalized_cat_df.empty:
                                category_flow = normalized_cat_df.groupby('category').agg({
                                    ...
                                    'volume_score': 'mean', 'rvol': 'mean'}).round(2)
                                
                                if not category_flow.empty:
                                    category_flow.columns = ['Avg Score', 'Count', 'Avg Momentum', 'Avg Volume', 'Avg RVOL']
                                    category_flow['Flow Score'] = (category_flow['Avg Score'] * 0.4 +
                                    ...
                                    fig_flow = go.Figure()
                                    fig_flow.add_trace(go.Bar(x=category_flow.index, y=category_flow['Flow Score'],
                                    ...
                                    fig_flow.update_layout(title=f"Smart Money Flow Direction: {flow_direction} (Dynamically Sampled)",
            ...
            if 'category_percentile' in wave_filtered_df.columns:
                ...
                for _, stock in close_to_leader.iterrows():
                    emergence_data.append({'Ticker': stock['ticker'], 'Company': stock['company_name'], 'Pattern': '🔥 CAT LEADER',
                        'Distance': f"{90 - stock['category_percentile']:.1f}% away", 'Current': f"{stock['category_percentile']:.1f}%ile", 'Score': stock['master_score']})
            ...
            if 'breakout_score' in wave_filtered_df.columns:
                ...
                for _, stock in close_to_breakout.iterrows():
                    emergence_data.append({'Ticker': stock['ticker'], 'Company': stock['company_name'], 'Pattern': '🎯 BREAKOUT',
                        'Distance': f"{80 - stock['breakout_score']:.1f} pts away", 'Current': f"{stock['breakout_score']:.1f} score", 'Score': stock['master_score']})
            ...
            if len(volume_surges) > 0:
                volume_surges['surge_score'] = (volume_surges['rvol_score'] * 0.5 +
                    volume_surges['volume_score'] * 0.3 + volume_surges['momentum_score'] * 0.2)
                ...
                with col1:
                    ...
                    rename_dict = {'ticker': 'Ticker', 'company_name': 'Company', 'rvol': 'RVOL', 'price': 'Price',
    ...
    with tabs[3]:
        ...
        if not filtered_df.empty:
            ...
            with col2:
                pattern_counts = {}; [pattern_counts.update({p: pattern_counts.get(p, 0) + 1}) for patterns in filtered_df['patterns'].dropna() if patterns for p in patterns.split(' | ')]
                if pattern_counts:
            ...
            if 'industry' in filtered_df.columns:
                ...
                if not industry_overview_df.empty:
                    ...
                    with ind_tab1:
                        ...
                        fig_industry.add_trace(go.Bar(x=top_industries.index[:15], y=top_industries['flow_score'][:15],
                            ...
                            hovertemplate=('Industry: %{x}<br>Flow Score: %{y:.1f}<br>Analyzed: %{customdata[0]} of %{customdata[1]} stocks<br>Sampling: %{customdata[2]:.1f}%<br>Avg Score: %{customdata[3]:.1f}<br>Median Score: %{customdata[4]:.1f}<extra></extra>'),
                            customdata=np.column_stack((top_industries['analyzed_stocks'][:15], top_industries['total_stocks'][:15],
                                top_industries['sampling_pct'][:15], top_industries['avg_score'][:15], top_industries['median_score'][:15]))))
                        fig_industry.update_layout(title="Top 15 Industries by Smart Money Flow", xaxis_title="Industry", yaxis_title="Flow Score", height=500, template='plotly_white', showlegend=False, xaxis_tickangle=-45)
                        ...
                        with col2:
                            ...
                            UIComponents.render_metric_card("Top 3 Avg Score", f"{top_3_avg:.1f}")
                        with col3:
                            strong_industries = len(industry_overview_df[industry_overview_df['flow_score'] > 60])
                            UIComponents.render_metric_card("Strong Industries", f"{strong_industries}", f"{strong_industries/len(industry_overview_df)*100:.0f}% of total")
                        with col4:
                            total_analyzed = industry_overview_df['analyzed_stocks'].sum()
                            UIComponents.render_metric_card("Stocks Analyzed", f"{total_analyzed:,}", f"From {len(filtered_df):,} total")
                    with ind_tab2:
                        ...
                        display_names = {'flow_score': 'Flow Score', 'avg_score': 'Avg Score', 'median_score': 'Median Score', 'avg_momentum': 'Avg Momentum', 'avg_volume': 'Avg Volume', 'avg_rvol': 'Avg RVOL', 'avg_ret_30d': 'Avg 30D Ret', 'analyzed_stocks': 'Analyzed', 'total_stocks': 'Total', 'sampling_pct': 'Sample %'}
            ...
            if not category_overview_df.empty:
                ...
                with cat_tab1:
                    ...
                    fig_category.add_trace(go.Bar(x=category_overview_df.index, y=category_overview_df['flow_score'],
                        text=[f"{val:.1f}" for val in category_overview_df['flow_score']], textposition='outside',
                        marker_color=bar_colors,
                        hovertemplate=('Category: %{x}<br>Flow Score: %{y:.1f}<br>Analyzed: %{customdata[0]} of %{customdata[1]} stocks<br>Avg Score: %{customdata[2]:.1f}<br>Avg Momentum: %{customdata[3]:.1f}<br>Avg Acceleration: %{customdata[4]:.1f}<extra></extra>'),
                        customdata=np.column_stack((category_overview_df['analyzed_stocks'], category_overview_df['total_stocks'], category_overview_df['avg_score'], category_overview_df['avg_momentum'], category_overview_df['avg_acceleration']))))
                    ...
                    with col2:
                        if 'avg_momentum' in category_overview_df.columns:
                            ...
                            if not highest_momentum.empty:
                                UIComponents.render_metric_card("Highest Momentum", f"{highest_momentum.index[0]}", f"{highest_momentum['avg_momentum'].iloc[0]:.1f}")
                    with col3:
                        if 'avg_acceleration' in category_overview_df.columns:
                            ...
                            if not highest_accel.empty:
                                UIComponents.render_metric_card("Best Acceleration", f"{highest_accel.index[0]}", f"{highest_accel['avg_acceleration'].iloc[0]:.1f}")
                with cat_tab2:
            ...
            if not search_results.empty:
                ...
                for idx, stock in search_results.iterrows():
                    with st.expander(f"📊 {stock['ticker']} - {stock['company_name']} (Rank #{int(stock['rank'])})", expanded=True):
                        ...
                        components = [("Position", stock['position_score'], CONFIG.POSITION_WEIGHT), ("Volume", stock['volume_score'], CONFIG.VOLUME_WEIGHT),
                            ("Momentum", stock['momentum_score'], CONFIG.MOMENTUM_WEIGHT), ("Acceleration", stock['acceleration_score'], CONFIG.ACCELERATION_WEIGHT),
                            ("Breakout", stock['breakout_score'], CONFIG.BREAKOUT_WEIGHT), ("RVOL", stock['rvol_score'], CONFIG.RVOL_WEIGHT)]
    ...
    with tabs[5]:
        ...
        export_stats = {"Total Stocks": len(filtered_df), "Average Score": f"{filtered_df['master_score'].mean():.1f}" if not filtered_df.empty else "N/A", "Stocks with Patterns": (filtered_df['patterns'] != '').sum() if 'patterns' in filtered_df.columns else 0, "High RVOL (>2x)": (filtered_df['rvol'] > 2).sum() if 'rvol' in filtered_df.columns else 0, "Positive 30D Returns": (filtered_df['ret_30d'] > 0).sum() if 'ret_30d' in filtered_df.columns else 0, "Data Quality": f"{RobustSessionState.safe_get('data_quality', {}).get('completeness', 0):.1f}%"}
