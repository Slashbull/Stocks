...
def load_and_process_data(source_type: str, file_data: Optional[BytesIO], 
                          spreadsheet_id: Optional[str], data_version: str) -> Tuple[pd.DataFrame, datetime, Dict[str, Any]]:
    """
    Load and process data with smart caching and versioning.
    The data_version parameter is a unique hash for cache invalidation.
    """
    ...
    try:
        if source_type == "upload" and file_data is not None:
            ...
            logger.info(f"Attempting to load data from Google Sheets with Spreadsheet ID: {spreadsheet_id}, GID: {CONFIG.DEFAULT_GID}")
...
class PatternDetector:
    ...
    def _get_all_pattern_definitions(df: pd.DataFrame) -> List[Tuple[str, pd.Series]]:
        ...
        def get_col_safe(col_name: str) -> pd.Series:
            return df[col_name].fillna(np.nan) if col_name in df.columns else pd.Series(np.nan, index=df.index)

        if 'category_percentile' in df.columns:
            mask = get_col_safe('category_percentile') >= CONFIG.PATTERN_THRESHOLDS['category_leader']
            patterns.append(('🔥 CAT LEADER', mask))
        
        if all(c in df.columns for c in ['category_percentile', 'percentile']):
            mask = (
                (get_col_safe('category_percentile') >= CONFIG.PATTERN_THRESHOLDS['hidden_gem']) & 
                (get_col_safe('percentile') < 70)
            )
        ...
        if all(c in df.columns for c in ['volume_score', 'vol_ratio_90d_180d']):
            mask = (
                (get_col_safe('volume_score') >= CONFIG.PATTERN_THRESHOLDS['institutional']) &
                (get_col_safe('vol_ratio_90d_180d') > 1.1)
            )
        ...
        if all(c in df.columns for c in ['momentum_score', 'acceleration_score']):
            mask = (
                (get_col_safe('momentum_score') >= CONFIG.PATTERN_THRESHOLDS['momentum_wave']) &
                (get_col_safe('acceleration_score') >= 70)
            )
            patterns.append(('🌊 MOMENTUM WAVE', mask))
        
        if all(c in df.columns for c in ['liquidity_score', 'percentile']):
            mask = (
                (get_col_safe('liquidity_score') >= CONFIG.PATTERN_THRESHOLDS['liquid_leader']) &
                (get_col_safe('percentile') >= CONFIG.PATTERN_THRESHOLDS['liquid_leader'])
            )
        ...
        if all(c in df.columns for c in ['pe', 'master_score']):
        ...
        if all(c in df.columns for c in ['eps_change_pct', 'acceleration_score']):
            ...
            mask = (
                (extreme_growth & (get_col_safe('acceleration_score') >= 80)) |
                (normal_growth & (get_col_safe('acceleration_score') >= 70))
            )
            patterns.append(('📊 EARNINGS ROCKET', mask))
        
        if all(c in df.columns for c in ['pe', 'eps_change_pct', 'percentile']):
            has_complete_data = (
                get_col_safe('pe').notna() & get_col_safe('eps_change_pct').notna() & 
                (get_col_safe('pe') > 0) & (get_col_safe('pe') < 10000)
            )
            mask = (
                has_complete_data & (get_col_safe('pe').between(10, 25)) &
                (get_col_safe('eps_change_pct') > 20) & (get_col_safe('percentile') >= 80)
            )
            patterns.append(('🏆 QUALITY LEADER', mask))
        
        if all(c in df.columns for c in ['eps_change_pct', 'volume_score']):
        ...
        if all(c in df.columns for c in ['from_high_pct', 'volume_score', 'momentum_score']):
            mask = (
                (get_col_safe('from_high_pct') > -5) & 
                (get_col_safe('volume_score') >= 70) & 
                (get_col_safe('momentum_score') >= 60)
            )
            patterns.append(('🎯 52W HIGH APPROACH', mask))
        
        if all(c in df.columns for c in ['from_low_pct', 'acceleration_score', 'ret_30d']):
            mask = (
                (get_col_safe('from_low_pct') < 20) & 
                (get_col_safe('acceleration_score') >= 80) & 
                (get_col_safe('ret_30d') > 10)
            )
            patterns.append(('🔄 52W LOW BOUNCE', mask))
        
        if all(c in df.columns for c in ['from_low_pct', 'from_high_pct', 'trend_quality']):
            mask = (
                (get_col_safe('from_low_pct') > 60) & 
                (get_col_safe('from_high_pct') > -40) & 
                (get_col_safe('trend_quality') >= 70)
            )
            patterns.append(('👑 GOLDEN ZONE', mask))
        
        if all(c in df.columns for c in ['vol_ratio_30d_90d', 'vol_ratio_90d_180d', 'ret_30d']):
            mask = (
                (get_col_safe('vol_ratio_30d_90d') > 1.2) & 
                (get_col_safe('vol_ratio_90d_180d') > 1.1) & 
                (get_col_safe('ret_30d') > 5)
            )
        ...
        if all(c in df.columns for c in ['ret_7d', 'ret_30d', 'acceleration_score', 'rvol']):
            ...
            mask = (
                ...
                (get_col_safe('acceleration_score') >= 85) & 
                (get_col_safe('rvol') > 2)
            )
        ...
        if all(c in df.columns for c in ['high_52w', 'low_52w', 'from_low_pct']):
            with np.errstate(divide='ignore', invalid='ignore'):
                range_pct = np.where(
                    get_col_safe('low_52w') > 0,
                    ((get_col_safe('high_52w') - get_col_safe('low_52w')) / get_col_safe('low_52w')) * 100,
                    100
                )
        ...
        if all(c in df.columns for c in ['vol_ratio_90d_180d', 'vol_ratio_30d_90d', 'from_low_pct', 'ret_7d', 'ret_30d']):
            ...
            mask = (
                (get_col_safe('vol_ratio_90d_180d') > 1.1) &
                (get_col_safe('vol_ratio_30d_90d').between(0.9, 1.1)) &
                (get_col_safe('from_low_pct') > 40) &
                ret_ratio.notna() & (ret_ratio > 1)
            )
        ...
        if all(c in df.columns for c in ['ret_1d', 'ret_7d', 'rvol', 'from_high_pct', 'category']):
            ...
            mask = (
                daily_pace_ratio.notna() & (daily_pace_ratio > 2) &
                (get_col_safe('rvol') > 3) &
                (get_col_safe('from_high_pct') > -15) &
                (df['category'].isin(['Small Cap', 'Micro Cap']))
            )
            patterns.append(('🧛 VAMPIRE', mask))
        
        if all(c in df.columns for c in ['momentum_harmony', 'master_score']):
            mask = (
                (get_col_safe('momentum_harmony') == 4) &
                (get_col_safe('master_score') > 80)
            )
            patterns.append(('⛈️ PERFECT STORM', mask))
        
        # All masks are created with NaNs handled and aligned to the main df index
        return [(name, mask.fillna(False)) for name, mask in patterns]
    ...
    def create_acceleration_profiles(df: pd.DataFrame, n: int = 10) -> go.Figure:
        ...
        try:
            ...
            for _, stock in accel_df.iterrows():
                ...
                if len(x_points) > 1:
                    ...
                    fig.add_trace(go.Scatter(
                        ...
                        hovertemplate=(f"<b>{stock['ticker']}</b><br>" + "%{x}: %{y:.1f}%<br>" + f"Accel Score: {accel_score:.0f}<extra></extra>")
                    ))
            ...
            fig.update_layout(
                ...
                xaxis_title="Time Frame", yaxis_title="Return %", height=400,
...
class ExportEngine:
    ...
    def create_excel_report(df: pd.DataFrame, template: str = 'full') -> BytesIO:
        ...
        try:
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                column_formats = {
                    'price': currency_format, 'master_score': score_format, 'position_score': score_format,
                    'volume_score': score_format, 'momentum_score': score_format, 'acceleration_score': score_format,
                    'breakout_score': score_format, 'rvol_score': score_format, 'trend_quality': score_format,
                    'pe': float_format, 'eps_current': float_format, 'from_low_pct': percent_format,
                    'from_high_pct': percent_format, 'ret_1d': percent_format, 'ret_3d': percent_format,
                    'ret_7d': percent_format, 'ret_30d': percent_format, 'ret_3m': percent_format,
                    'ret_6m': percent_format, 'ret_1y': percent_format, 'ret_3y': percent_format,
                    'ret_5y': percent_format, 'eps_change_pct': percent_format, 'rvol': rvol_format,
                    'vmi': float_format, 'money_flow_mm': currency_m_format, 'position_tension': float_format,
                    'momentum_harmony': integer_format, 'overall_wave_strength': score_format
                }
                ...
                ad_ratio_val = ad_metrics.get('ad_ratio', 1)
                ad_ratio_str = f"{ad_ratio_val:.2f}" if ad_ratio_val != float('inf') else 'Infinity'
                intel_data.append({
                    'Metric': 'Advance/Decline Ratio (1D)', 'Value': ad_ratio_str,
                ...
                if pattern_counts:
                    pattern_df = pd.DataFrame(
                        list(pattern_counts.items()), columns=['Pattern', 'Count']
                ...
                wave_signals = df[
                    (df['momentum_score'].fillna(0) >= 60) & (df['acceleration_score'].fillna(0) >= 70) &
                    (df['rvol'].fillna(0) >= 2)
                ].nlargest(50, 'master_score', keep='first')
                
                if not wave_signals.empty:
                    wave_cols = ['ticker', 'company_name', 'master_score', 'momentum_score', 
                                'acceleration_score', 'rvol', 'wave_state', 'patterns', 'category', 'sector', 'industry']
    ...
    def create_csv_export(df: pd.DataFrame) -> str:
        ...
        export_cols = [
            'rank', 'ticker', 'company_name', 'master_score', 'position_score', 'volume_score',
            'momentum_score', 'acceleration_score', 'breakout_score', 'rvol_score', 'trend_quality',
            'price', 'pe', 'eps_current', 'eps_change_pct', 'from_low_pct', 'from_high_pct',
            'ret_1d', 'ret_7d', 'ret_30d', 'ret_3m', 'ret_6m', 'ret_1y', 'rvol', 'vmi',
            'money_flow_mm', 'position_tension', 'momentum_harmony', 'wave_state', 'patterns', 
...
# ============================================
# SESSION STATE MANAGER AND AUTHENTICATION
# ============================================
...
class SessionStateManager:
    ...
    def initialize():
        ...
        defaults = {
            'wd_is_authenticated': False,
            'wd_search_query': "",
            'last_refresh': datetime.now(timezone.utc),
            'data_source': "sheet",
            'user_preferences': {'default_top_n': CONFIG.DEFAULT_TOP_N, 'display_mode': 'Technical', 'last_filters': {}},
            'filters': {}, 'active_filter_count': 0, 'quick_filter': None, 'wd_quick_filter_applied': False,
            'wd_show_debug': False, 'performance_metrics': {}, 'data_quality': {}, 'wd_trigger_clear': False,
            'wd_category_filter': [], 'wd_sector_filter': [], 'wd_industry_filter': [], 'wd_min_score': 0,
            'wd_patterns': [], 'wd_trend_filter': "All Trends", 'wd_eps_tier_filter': [], 'wd_pe_tier_filter': [],
            'wd_price_tier_filter': [], 'wd_min_eps_change': "", 'wd_min_pe': "", 'wd_max_pe': "",
            'wd_require_fundamental_data': False, 'wd_wave_states_filter': [], 'wd_wave_strength_range_slider': (0, 100),
            'wd_show_sensitivity_details': False, 'wd_show_market_regime': True, 'wd_wave_timeframe_select': "All Waves",
            'wd_wave_sensitivity': "Balanced", 'user_spreadsheet_id': None, 'wd_current_page_rankings': 0
        }
    ...
    def clear_filters():
        ...
        filter_keys = [
            'wd_category_filter', 'wd_sector_filter', 'wd_industry_filter', 'wd_pe_tier_filter',
            'wd_price_tier_filter', 'wd_patterns', 'wd_min_score', 'wd_trend_filter',
            'wd_min_eps_change', 'wd_min_pe', 'wd_max_pe', 'wd_require_fundamental_data',
            'quick_filter', 'wd_quick_filter_applied', 'wd_wave_states_filter',
            'wd_wave_strength_range_slider', 'wd_show_sensitivity_details',
            'wd_show_market_regime', 'wd_wave_timeframe_select', 'wd_wave_sensitivity',
        ]
    ...
    def authenticate(username, password):
        """Authenticates user with hardcoded credentials."""
        if username == 'gemini' and password == 'gemini':
            st.session_state.wd_is_authenticated = True
...
def login_page():
    ...
    st.title("Login to Wave Detection Ultimate 3.0")
    ...
    with st.form("login_form"):
        ...
        if st.form_submit_button("Login"):
            SessionStateManager.authenticate(username, password)
            
def main_app():
    """Main Streamlit application after authentication."""
    ...
    @media (max-width: 768px) {
        .stDataFrame {font-size: 12px;} div[data-testid="metric-container"] {padding: 3%;}
    ...
    with st.sidebar:
        ...
        if st.session_state.data_source == "upload":
            uploaded_file = st.file_uploader(
                ...
                help="Upload a CSV file with stock data. Must contain 'ticker' and 'price' columns.", key="wd_csv_uploader"
            )
        ...
        if st.session_state.data_source == "sheet":
            ...
            user_gid_input_widget = st.text_input(
                "Enter Google Spreadsheet ID", value=current_gid_input_value,
                placeholder=f"e.g., 1OEQ_qxL4lXbO9LlKWDGlD...",
                help="The unique ID from your Google Sheet URL.", key="wd_user_gid_input"
            )
        ...
        filter_checks = [
            ('wd_category_filter', lambda x: x and len(x) > 0), ('wd_sector_filter', lambda x: x and len(x) > 0),
            ('wd_industry_filter', lambda x: x and len(x) > 0), ('wd_min_score', lambda x: x > 0),
            ('wd_patterns', lambda x: x and len(x) > 0), ('wd_trend_filter', lambda x: x != 'All Trends'),
            ('wd_eps_tier_filter', lambda x: x and len(x) > 0), ('wd_pe_tier_filter', lambda x: x and len(x) > 0),
            ('wd_price_tier_filter', lambda x: x and len(x) > 0), ('wd_min_eps_change', lambda x: x is not None and str(x).strip() != ''),
            ('wd_min_pe', lambda x: x is not None and str(x).strip() != ''), ('wd_max_pe', lambda x: x is not None and str(x).strip() != ''),
            ('wd_require_fundamental_data', lambda x: x), ('wd_wave_states_filter', lambda x: x and len(x) > 0),
    ...
    try:
        ...
        with st.spinner("📥 Loading and processing data..."):
            try:
                if st.session_state.data_source == "upload":
                    ranked_df, data_timestamp, metadata = load_and_process_data("upload", file_data=uploaded_file, spreadsheet_id=None, data_version=cache_data_version)
                else:
                    ranked_df, data_timestamp, metadata = load_and_process_data("sheet", file_data=None, spreadsheet_id=active_gid_for_load, data_version=cache_data_version)
                ...
                if 'last_good_data' in st.session_state:
                    ...
                    st.warning(f"Failed to load fresh data, using cached version. Error: {str(e)}")
                else:
    ...
    if quick_filter and ranked_df is not None and not ranked_df.empty:
        ...
        else: ranked_df_display = ranked_df.copy(); st.warning(f"Data for '{quick_filter}' filter not available.")
    ...
    if show_debug:
        with st.sidebar.expander("🐛 Debug Info", expanded=True):
            ...
            for key, value in filters.items():
                if value is not None and value != [] and not (isinstance(value, (int, float)) and value == 0) and not (isinstance(value, str) and value == "") and not (isinstance(value, tuple) and value == (0,100)):
                    st.write(f"• {key}: {value}")
            st.write(f"\n**Filter Result:**")
            st.write(f"Before: {len(ranked_df)} stocks")
            st.write(f"After: {len(filtered_df)} stocks")
            clipping_counts = DataValidator.get_clipping_counts()
            if clipping_counts: st.write("\n**Data Clipping Events:**"); st.json(clipping_counts)
    ...
    with col3:
        if show_fundamentals and 'pe' in filtered_df.columns:
            ...
            if pe_coverage > 0:
                ...
                UIComponents.render_metric_card("Median PE", f"{median_pe:.1f}x", f"{pe_pct:.0f}% have data", "Median Price-to-Earnings ratio for stocks with valid PE data.")
        ...
        if not display_df.empty:
            ...
            with st.expander("📊 Quick Statistics"):
                ...
                with stat_cols[0]:
                    ...
                    if 'master_score' in filtered_df.columns: scores_data = filtered_df['master_score'].dropna()
                    else: scores_data = pd.Series([])
                    if not scores_data.empty: st.text(f"Max: {scores_data.max():.1f}"); st.text(f"Min: {scores_data.min():.1f}")
                with stat_cols[1]:
                    st.markdown("**Returns (30D)**")
                    if 'ret_30d' in filtered_df.columns: returns_data = filtered_df['ret_30d'].dropna()
                    else: returns_data = pd.Series([])
                    if not returns_data.empty: st.text(f"Max: {returns_data.max():.1f}%"); st.text(f"Min: {returns_data.min():.1f}%")
                with stat_cols[2]:
                    if show_fundamentals:
                        st.markdown("**Fundamentals**")
                        if 'pe' in filtered_df.columns: valid_pe = filtered_df['pe'].dropna(); valid_pe = valid_pe[(valid_pe > 0) & (valid_pe < 10000)]; st.text(f"Median PE: {valid_pe.median():.1f}x") if not valid_pe.empty else st.text("No valid PE.")
                    else:
                        st.markdown("**Volume**")
                        if 'rvol' in filtered_df.columns: rvol_data = filtered_df['rvol'].dropna(); st.text(f"Max: {rvol_data.max():.1f}x"); st.text(f"Avg: {rvol_data.mean():.1f}x")
                with stat_cols[3]:
    ...
    with tabs[2]:
        ...
        with radar_col4:
            if not wave_filtered_df.empty and 'overall_wave_strength' in wave_filtered_df.columns:
                try:
                    ...
                    logger.error(f"Error calculating wave strength: {str(e)}"); UIComponents.render_metric_card("Wave Strength", "N/A", "Error calculating strength.")
        ...
        if wave_timeframe != "All Waves":
            try:
                if wave_timeframe == "Intraday Surge":
                    if all(col in wave_filtered_df.columns for col in ['rvol', 'ret_1d', 'price', 'prev_close']): wave_filtered_df = wave_filtered_df[(wave_filtered_df['rvol'].fillna(0) >= 2.5) & (wave_filtered_df['ret_1d'].fillna(0) > 2) & (wave_filtered_df['price'].fillna(0) > wave_filtered_df['prev_close'].fillna(0) * 1.02)]
                    ...
                    if all(col in wave_filtered_df.columns for col in ['ret_3d', 'vol_ratio_7d_90d', 'price', 'sma_20d']): wave_filtered_df = wave_filtered_df[(wave_filtered_df['ret_3d'].fillna(0) > 5) & (wave_filtered_df['vol_ratio_7d_90d'].fillna(0) > 1.5) & (wave_filtered_df['price'].fillna(0) > wave_filtered_df['sma_20d'].fillna(0))]
                    else: st.warning(f"Required data for '{wave_timeframe}' not available. Showing all waves."); wave_filtered_df = filtered_df.copy()
                elif wave_timeframe == "Weekly Breakout":
                    if all(col in wave_filtered_df.columns for col in ['ret_7d', 'vol_ratio_7d_90d', 'from_high_pct']): wave_filtered_df = wave_filtered_df[(wave_filtered_df['ret_7d'].fillna(0) > 8) & (wave_filtered_df['vol_ratio_7d_90d'].fillna(0) > 2.0) & (wave_filtered_df['from_high_pct'].fillna(-100) > -10)]
                    else: st.warning(f"Required data for '{wave_timeframe}' not available. Showing all waves."); wave_filtered_df = filtered_df.copy()
                elif wave_timeframe == "Monthly Trend":
                    if all(col in wave_filtered_df.columns for col in ['ret_30d', 'price', 'sma_20d', 'sma_50d', 'vol_ratio_30d_180d', 'from_low_pct']): wave_filtered_df = wave_filtered_df[(wave_filtered_df['ret_30d'].fillna(0) > 15) & (wave_filtered_df['price'].fillna(0) > wave_filtered_df['sma_20d'].fillna(0)) & (wave_filtered_df['sma_20d'].fillna(0) > wave_filtered_df['sma_50d'].fillna(0)) & (wave_filtered_df['vol_ratio_30d_180d'].fillna(0) > 1.2) & (wave_filtered_df['from_low_pct'].fillna(0) > 30)]
                    else: st.warning(f"Required data for '{wave_timeframe}' not available. Showing all waves."); wave_filtered_df = filtered_df.copy()
            except Exception as e:
        ...
        if not wave_filtered_df.empty:
            ...
            if not momentum_shifts.empty:
                ...
                display_columns.extend(['category', 'sector', 'industry'])
                ...
                with col1:
                    try:
                        if 'category' in wave_filtered_df.columns:
                            category_dfs = [MarketIntelligence._apply_dynamic_sampling(wave_filtered_df[wave_filtered_df['category'] == cat].copy()) for cat in wave_filtered_df['category'].dropna().unique() if cat != 'Unknown']
                            if category_dfs:
                                normalized_cat_df = pd.concat([df for df in category_dfs if not df.empty], ignore_index=True)
                    ...
                    except Exception as e: logger.error(f"Error in category flow analysis: {str(e)}"); st.error("Unable to analyze category flow")
                with col2:
    ...
    with tabs[3]:
        ...
        if not filtered_df.empty:
            ...
            with col2:
                pattern_counts = {}; [pattern_counts.update({p: pattern_counts.get(p, 0) + 1}) for patterns_str in filtered_df['patterns'].dropna() if patterns_str for p in patterns_str.split(' | ')]
                if pattern_counts:
            ...
            if 'category' in filtered_df.columns:
                category_df_agg = filtered_df.groupby('category').agg({'master_score': ['mean', 'count'], 'category_percentile': 'mean', 'money_flow_mm': 'sum' if 'money_flow_mm' in filtered_df.columns else lambda x: 0}).round(2)
    ...
    with tabs[4]:
        ...
        with col1:
            search_query = st.text_input("Search stocks", value=st.session_state.get('wd_search_query', ''), placeholder="Enter ticker or company name...", help="Search by ticker symbol or company name", key="wd_search_input")
            ...
            if not search_results.empty:
                ...
                for idx, stock in search_results.iterrows():
                    with st.expander(f"📊 {stock['ticker']} - {stock['company_name']} (Rank #{int(stock['rank']) if pd.notna(stock['rank']) else 'N/A'})", expanded=True):
                        ...
                        with metric_cols[1]: UIComponents.render_metric_card("Price", f"₹{stock['price']:,.0f}" if pd.notna(stock.get('price')) else "N/A", f"{stock['ret_1d']:+.1f}%" if pd.notna(stock.get('ret_1d')) else None, help_text="Current stock price and 1-day return.")
                        ...
                        for i, (name, score, weight, help_text_comp) in enumerate(components):
                            with score_cols_breakdown[i]:
                                color = "⚪" if pd.isna(score) else "🟢" if score >= 80 else "🟡" if score >= 60 else "🔴"
                                display_score = "N/A" if pd.isna(score) else f"{score:.0f}"
                        ...
                        with detail_cols_top[0]:
                            ...
                            if show_fundamentals:
                                st.markdown("**💰 Fundamentals**")
                                if 'pe' in stock and pd.notna(stock['pe']): pe_val = stock['pe']; st.text("PE Ratio: 🔴 Loss") if pe_val <= 0 else st.text(f"PE Ratio: 🟢 {pe_val:.1f}x") if pe_val < 15 else st.text(f"PE Ratio: 🟡 {pe_val:.1f}x") if pe_val < 25 else st.text(f"PE Ratio: 🔴 {pe_val:.1f}x")
                                ...
                                if 'eps_change_pct' in stock and pd.notna(stock['eps_change_pct']): eps_chg = stock['eps_change_pct']; st.text(f"EPS Growth: 🚀 {eps_chg:+.0f}%") if eps_chg >= 100 else st.text(f"EPS Growth: 🔥 {eps_chg:+.1f}%") if eps_chg >= 50 else st.text(f"EPS Growth: 📈 {eps_chg:+.1f}%") if eps_chg >= 0 else st.text(f"EPS Growth: 📉 {eps_chg:+.1f}%")
                        ...
                        with detail_cols_tech[0]:
                            ...
                            if all(col in stock.index and pd.notna(stock[col]) for col in ['low_52w', 'high_52w']):
                                st.text(f"52W Low: ₹{stock.get('low_52w', 0):,.0f}"); st.text(f"52W High: ₹{stock.get('high_52w', 0):,.0f}")
                        ...
                        with detail_cols_tech[1]:
                            ...
                            with adv_col1:
                                ...
                                if 'momentum_harmony' in stock and pd.notna(stock['momentum_harmony']): harmony_val = stock['momentum_harmony']; harmony_emoji = "🟢" if harmony_val >= 3 else "🟡" if harmony_val >= 2 else "🔴"; st.metric("Harmony", f"{harmony_emoji} {int(harmony_val)}/4", help_text="Momentum Harmony: measures alignment of returns across multiple timeframes.")
                                else: st.metric("Harmony", "N/A")
                            with adv_col2:
                                if 'position_tension' in stock and pd.notna(stock['position_tension']): st.metric("Position Tension", f"{stock['position_tension']:.0f}", help_text="Measures the stock's position within its 52-week range relative to its volatility.")
    ...
    with tabs[5]:
        ...
        stat_cols = st.columns(3); [UIComponents.render_metric_card(label, value) for i, (label, value) in enumerate(export_stats.items()) for with_cols in [stat_cols[i % 3]]];
...
if __name__ == "__main__":
    SessionStateManager.initialize()
    if st.session_state.wd_is_authenticated:
        main_app()
    else:
        login_page()
